...
function applyBossNotifyCollapse(collapsed){const isCollapsed=collapsed===true;if(homeEl.panel)homeEl.panel.classList.toggle("isCollapsed",isCollapsed);if(homeEl.content)homeEl.content.hidden=isCollapsed;if(homeEl.collapse){homeEl.collapse.setAttribute("aria-expanded",String(!isCollapsed));const label=isCollapsed?"Expand boss notifications":"Collapse boss notifications";homeEl.collapse.title=label;const accessibleLabel=homeEl.collapse.querySelector(".srOnly");if(accessibleLabel)accessibleLabel.textContent=label}}
