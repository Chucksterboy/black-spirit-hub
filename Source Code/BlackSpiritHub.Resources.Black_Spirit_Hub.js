...
const BRIDGE_TIMEOUTS={selectGrindLootImage:120000,scanGrindLootImage:120000,downloadAndInstallUpdate:600000,refreshEvents:105000,initializeEvents:105000};
...
function normalizedHomeSettings(){const saved=readSetting("homeSettings",{});return {timeFormat:saved.timeFormat==="24"?"24":"12",showLocalTime:saved.showLocalTime===true,masterNotifications:saved.masterNotifications!==false,leadMinutes:[5,10,15,30].includes(Number(saved.leadMinutes))?Number(saved.leadMinutes):15,soundEnabled:saved.soundEnabled!==false,ttsEnabled:saved.ttsEnabled===true,guildBossNotifications:saved.guildBossNotifications===true,bosses:{...defaultBossSelection(),...(saved.bosses||{})},notified:saved.notified&&typeof saved.notified==="object"?saved.notified:{}}}
...
function renderBossToggles(settings){if(!homeEl.toggles)return;homeEl.toggles.innerHTML=homeBossScheduleState.bosses.map(b=>{const enabled=settings.bosses[b]!==false;return `<label class="bossToggle ${enabled?"":"disabledBoss"}"><input type="checkbox" data-boss-toggle="${escapeHtml(b)}" ${enabled?"checked":""}><span class="bossName ${bossClass(b)}">${escapeHtml(b)}</span></label>`}).join("")}
...
function updateHomeTimers(settings=normalizedHomeSettings()){const now=new Date(),next=nextBossSpawn(now);if(next){const bossLabel=next.bosses.join(" + "),countdown=fmtCountdown(next.date-now);homeEl.nextBossTime.textContent=countdown;if(homeEl.nextBossTitle){homeEl.nextBossTitle.textContent=`Next Boss: ${bossLabel}`;homeEl.nextBossTitle.className=`homeTimerTitle ${next.bosses.map(bossClass).join(" ")}`}homeEl.nextBossSub.textContent=`${fmtSpawnDateTime(next,settings)} - ${HOME_TIMER_CONFIG.region}`;homeEl.nextBossLine.textContent=`Next: ${bossLabel} in ${countdown} (${fmtSpawnDateTime(next,settings)})`}const dn=dayNightState(now);homeEl.dayNightValue.textContent=dn.state;homeEl.dayNightSub.textContent=`${fmtCountdown(dn.remain)} until ${dn.next}`;homeEl.dayNightProgress.style.width=`${Math.round(dn.progress*100)}%`;updateGuildBossTimer(now);updateBossScheduleStatus()}
const homeAlertInFlight=new Set();
let homeAlertRunPromise=null;
function alertStage(settings,delta,keyBase){const lead=Number(settings.leadMinutes||15);return delta>=0&&delta<=lead*60000&&!(settings.notified||{})[keyBase]?Math.max(1,Math.ceil(delta/60000)):null}
...
function nextAlertableBossSpawn(settings,now=new Date()){for(const spawn of allBossSpawns(now)){if(spawn.date<=now)continue;const bosses=spawn.bosses.filter(boss=>settings.bosses[boss]!==false);if(bosses.length)return {...spawn,bosses}}return null}
...
async function persistDeliveredHomeAlert(key,settings,deliver){if(homeAlertInFlight.has(key))return false;homeAlertInFlight.add(key);try{const delivered=await deliver();if(delivered){settings.notified={...settings.notified,[key]:true};pruneHomeNotifications(settings);saveHomeSettings(settings)}return delivered}finally{homeAlertInFlight.delete(key)}}
async function checkBossNotifications(settings,now,next){if(!settings.masterNotifications||!next)return false;const delta=next.date-now,key=`boss|${next.date.toISOString()}`,stage=alertStage(settings,delta,key);if(!stage)return false;console.debug("[HomeBossNotify]",{bosses:next.bosses,spawnUtc:next.date.toISOString(),deltaMs:delta,warningMinutes:stage,leadMinutes:settings.leadMinutes,serverTime:fmtSpawnDateTime(next,{...settings,showLocalTime:false}),localTime:fmtSpawnDateTime(next,{...settings,showLocalTime:true})});const title=`${next.bosses.join(" + ")} spawning soon`,message=`${fmtSpawnDateTime(next,settings)} - ${stage} minute warning - ${HOME_TIMER_CONFIG.region}`,spoken=`${spokenBossList(next.bosses)} spawning in ${alertLeadText(stage)}.`;return persistDeliveredHomeAlert(key,settings,()=>sendHomeAlert(title,message,spoken,settings))}
async function checkGuildBossNotifications(settings,now){if(!settings.masterNotifications||!settings.guildBossNotifications)return false;const target=guildBossTarget(now);if(!target)return false;const delta=target.date-now,key=`guild|${target.date.toISOString()}`,stage=alertStage(settings,delta,key);if(!stage)return false;const title="Guild bosses spawning soon",message=`${guildBossDayName(target.day)} ${target.time} - ${stage} minute warning - Weekly guild boss timer`,spoken=`Guild bosses spawning in ${alertLeadText(stage)}.`;return persistDeliveredHomeAlert(key,settings,()=>sendHomeAlert(title,message,spoken,settings))}
...
homeEl.lead?.addEventListener("change",()=>{const s=normalizedHomeSettings();s.leadMinutes=Number(homeEl.lead.value);commitHomeSettings(s);NotificationService.ShowInfo(`Boss warning time set to ${s.leadMinutes} minutes.`,"Home settings saved")});
...
homeEl.testTts?.addEventListener("click",()=>{const settings=normalizedHomeSettings(),next=nextAlertableBossSpawn(settings),text=next?`${spokenBossList(next.bosses)} spawning in ${alertLeadText(settings.leadMinutes)}.`:"Black Spirit Hub text to speech test.";runBossAlertTest(homeEl.testTts,"Speaking",()=>bridgeCall("speakText",{text}),()=>`TTS played successfully: ${text}`)});
...
function grindRecoverySessions(){const current=grindNormalizeSessions(readSetting("grindTrackerSessionsRecovery",[]));if(current.length)return current;try{const legacy=grindNormalizeSessions(JSON.parse(localStorage.getItem("grindTrackerSessionsRecovery")||"[]"));if(legacy.length){settingMemory.set("grindTrackerSessionsRecovery",legacy);localStorage.setItem("blackSpiritHub.grindTrackerSessionsRecovery",JSON.stringify(legacy));localStorage.removeItem("grindTrackerSessionsRecovery")}return legacy}catch{return[]}}
function grindPersistRecovery(rows){try{const recovery=rows.slice(0,250),json=JSON.stringify(recovery);if(json.length<1500000){settingMemory.set("grindTrackerSessionsRecovery",recovery);localStorage.setItem("blackSpiritHub.grindTrackerSessionsRecovery",json);localStorage.removeItem("grindTrackerSessionsRecovery")}}catch(error){console.warn("[GrindTracker] Could not update the local recovery copy.",error)}}
