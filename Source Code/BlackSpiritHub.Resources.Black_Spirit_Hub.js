...
const RECIPE_BOOK_OCR_RELEVANT_ICON_MIN_SCORE=.70,RECIPE_BOOK_OCR_RELEVANT_CLASS_MARGIN=.01;
...
function recipeBookOcrPartitionReviewRows(analysis,data){
  ...
  for(const slot of analysis.slots){
    const pureEligible=slot.iconCandidates.filter(candidate=>{const classification=iconClasses.get(candidate.icon);return classification?.eligible&&!classification.negative}),pureNegative=slot.iconCandidates.filter(candidate=>{const classification=iconClasses.get(candidate.icon);return classification?.negative&&!classification.eligible}),mixed=slot.iconCandidates.filter(candidate=>{const classification=iconClasses.get(candidate.icon);return classification?.eligible&&classification.negative}),bestEligible=pureEligible[0]||null,bestNegative=pureNegative[0]||null,bestMixed=mixed[0]||null,eligibleScore=bestEligible?.score||0,negativeScore=bestNegative?.score||0,mixedScore=bestMixed?.score||0,oppositionScore=Math.max(negativeScore,mixedScore),relevance=bestEligible&&eligibleScore>=RECIPE_BOOK_OCR_RELEVANT_ICON_MIN_SCORE&&eligibleScore-oppositionScore>=RECIPE_BOOK_OCR_RELEVANT_CLASS_MARGIN?"eligible":bestNegative&&negativeScore-Math.max(eligibleScore,mixedScore)>=RECIPE_BOOK_OCR_RELEVANT_CLASS_MARGIN?"negative":"uncertain",matchedIcon=relevance==="eligible"?bestEligible:slot.iconCandidates.find(candidate=>iconIndex.has(candidate.icon))||null,nextIcon=matchedIcon?slot.iconCandidates.find(candidate=>candidate!==matchedIcon)||null:null,topResources=matchedIcon?(iconIndex.get(matchedIcon.icon)||[]):[],gap=matchedIcon?matchedIcon.score-(nextIcon?.score||0):0,qualityFamily=recipeBookOcrQualityFamilyResources(topResources,data),trustedBorderGrade=Number.isInteger(slot.borderGrade)&&slot.borderGrade>=0&&slot.borderGrade<=2&&slot.borderGradeConfidence>=RECIPE_BOOK_OCR_BORDER_GRADE_CONFIDENCE&&qualityFamily.length>1,borderGradeMatches=trustedBorderGrade?qualityFamily.filter(resource=>data.items[resource.itemId]?.grade===slot.borderGrade):[],borderGradeResolved=trustedBorderGrade&&borderGradeMatches.length===1,borderGradeConflict=trustedBorderGrade&&!borderGradeResolved,resolvedKey=borderGradeResolved?borderGradeMatches[0].key:"",materialExactResources=trustedBorderGrade?borderGradeMatches:topResources,optionMap=new Map();
    ...
    (relevance==="eligible"?rows:ignoredRows).push(row);
  }
