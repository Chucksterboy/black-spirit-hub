...
const couponEl={search:document.getElementById("couponSearch"),status:document.getElementById("couponStatusFilter"),showExpired:document.getElementById("couponShowExpired"),refresh:document.getElementById("couponRefresh"),message:document.getElementById("couponMessage"),rows:document.getElementById("couponRows"),availableTab:document.getElementById("couponAvailableTabCount"),expired:document.getElementById("couponExpiredCount"),expiredTab:document.getElementById("couponExpiredTabCount"),redeemed:document.getElementById("couponRedeemedCount"),sort:document.getElementById("couponSort"),detail:document.getElementById("couponDetail"),statusAlert:document.getElementById("couponStatusAlert"),pagePrevious:document.getElementById("couponPagePrevious"),pageNext:document.getElementById("couponPageNext"),pageStatus:document.getElementById("couponPageStatus")};
...
function couponNormalizeRedeemedMap(saved){const normalized={};if(saved&&typeof saved==="object")Object.entries(saved).forEach(([code,redeemed])=>{const key=couponCodeKey(code);if(key&&redeemed===true)normalized[key]=true});return normalized}
function couponRedeemedMapFromCodes(codes){return couponNormalizeRedeemedMap(Object.fromEntries((Array.isArray(codes)?codes:[]).filter(code=>typeof code==="string").map(code=>[code,true])))}
let couponRedeemedState=couponNormalizeRedeemedMap(readSetting("couponRedeemed",{}));
let couponRedemptionStateInitialized=false;
let couponRedemptionWriteQueue=Promise.resolve();
let couponRedemptionWriteGeneration=0;
function couponRedeemedMap(){return {...couponRedeemedState}}
function couponIsRedeemed(code){return couponRedeemedMap()[couponCodeKey(code)]===true}
function couponRedeemedCodes(){return Object.keys(couponRedeemedState).sort((a,b)=>a.localeCompare(b))}
function couponPendingRedemptionMap(){const pending=readSetting("couponRedemptionPending",null);return pending&&Array.isArray(pending.redeemedCodes)?couponRedeemedMapFromCodes(pending.redeemedCodes):null}
function couponQueueRedemptionSave({notify=false}={}){const redeemedCodes=couponRedeemedCodes(),generation=++couponRedemptionWriteGeneration;persistSetting("couponRedemptionPending",{schemaVersion:1,redeemedCodes});flushSetting("couponRedemptionPending");couponRedemptionWriteQueue=couponRedemptionWriteQueue.catch(()=>null).then(()=>bridgeCall("saveCouponRedemptions",{redeemedCodes})).then(result=>{if(generation===couponRedemptionWriteGeneration&&JSON.stringify(couponRedeemedCodes())===JSON.stringify(redeemedCodes)){persistSetting("couponRedemptionPending",null);flushSetting("couponRedemptionPending");}return result});couponRedemptionWriteQueue.catch(error=>{console.warn("[Coupons] durable redemption save failed",error);if(notify)NotificationService.ShowWarning("Redeemed status is saved locally, but its update-safe backup could not be saved.","Coupons");});return couponRedemptionWriteQueue}
function couponInitializeRedemptionState(data){if(couponRedemptionStateInitialized)return;couponRedemptionStateInitialized=true;const pending=couponPendingRedemptionMap();if(pending!==null)couponRedeemedState=pending;else if(data?.redemptionStateExists===true)couponRedeemedState=couponRedeemedMapFromCodes(data.redeemedCodes);persistSetting("couponRedeemed",couponRedeemedState);flushSetting("couponRedeemed");if(pending!==null||data?.redemptionStateExists!==true)couponQueueRedemptionSave()}
...
function setCouponRedeemed(code,redeemed){const map=couponRedeemedMap(),key=couponCodeKey(code);if(!key)return;if(redeemed)map[key]=true;else delete map[key];couponRedeemedState=map;persistSetting("couponRedeemed",map);flushSetting("couponRedeemed");if(redeemed)couponClearNewCode(key);couponQueueRedemptionSave({notify:true})}
...
function openAvailableCoupons(){const button=document.querySelector('[data-app-view="couponsView"]');if(!button)return false;const available=document.querySelector('[data-coupon-tab="available"]');if(couponState.activeTab!=="available")available?.click();activateAppView(button);return true}
...
function couponExpiryBadge(c){const date=new Date(c?.expiryUtc);if(!c?.expiryUtc||Number.isNaN(date.getTime()))return'<span class="couponCodeExpiry unknown">EXPIRY NOT LISTED</span>';const exact=date.toLocaleDateString([],{year:"numeric",month:"short",day:"numeric",timeZone:"UTC"}),relative=couponExpiryText(c.expiryUtc,c.isExpired,c.expiryText),relativeLabel=c.isExpired?relative:relative==="Expires today"?"TODAY":`IN ${relative.toUpperCase()}`;return`<span class="couponCodeExpiry ${c.isExpired?"expired":""}" title="${couponEscape(date.toLocaleString([],{timeZone:"UTC",timeZoneName:"short"}))}">${couponEscape(c.isExpired?`EXPIRED ${exact}`:`EXPIRES ${exact} · ${relativeLabel}`)}</span>`}
function applyCouponDashboard(data,{checkNew=false,silent=false}={}){couponInitializeRedemptionState(data);couponState.coupons=Array.isArray(data.coupons)?data.coupons:[];couponEl.search.value="";couponEl.status.value="all";couponEl.sort.value="newest";couponEl.showExpired.checked=true;const available=couponVisibleAvailable(),redeemed=couponVisibleRedeemed();if(couponEl.availableTab)couponEl.availableTab.textContent=available.length;couponEl.expired.textContent=data.expiredCount??couponState.coupons.filter(x=>x.isExpired).length;if(couponEl.redeemed)couponEl.redeemed.textContent=redeemed.length;const status=String(data.status||"CACHED").toUpperCase();couponEl.message.textContent=data.message||((status!=="LIVE"&&data.isStale)?"Showing stored coupon data from the last successful refresh.":"");if(checkNew)couponNotifyNewCodes(couponState.coupons,{silent});else{if(!couponState.lastKnownCodes.size)couponRememberCodes(couponState.coupons);couponPruneNewCouponAlert();}if(!couponState.selectedCode||!couponState.coupons.some(c=>c.code===couponState.selectedCode))couponState.selectedCode=(couponState.activeTab==="redeemed"?redeemed[0]?.code:available[0]?.code)||couponState.coupons[0]?.code||"";renderCoupons();}
function renderCoupons(){
  ...
  couponEl.rows.innerHTML=pageRows.map(c=>{
    ...
    return`<article class="couponRowV2 ${c.code===couponState.selectedCode?"selected":""}" data-coupon-code="${couponEscape(c.code)}"><div class="couponRowCode"><span class="couponTicket" aria-hidden="true">&#9670;</span><span class="couponCodeIdentity"><span class="couponCodeText" title="${couponEscape(c.code)}">${couponEscape(c.code)}</span>${couponExpiryBadge(c)}</span></div><div class="couponRedeemCell">${couponRedeemButton(c)}</div><div class="couponRewardSummary">${first?`<img src="${couponEscape(first.icon)}" alt=""><span>${couponEscape(first.quantity)}x ${couponEscape(first.itemName)}${rewards.length>1?`<br>+${rewards.length-1} items`:""}</span>`:"Rewards unavailable"}</div><div class="couponChevron">&rsaquo;</div></article>`;
  }).join("");
  renderCouponDetail(pageRows.find(c=>c.code===couponState.selectedCode)||pageRows[0]);
}
async function copyCouponCodeToClipboard(code){try{await navigator.clipboard.writeText(code);return true}catch{}const input=document.createElement("textarea");input.value=code;input.setAttribute("readonly","");input.style.position="fixed";input.style.opacity="0";try{document.body.appendChild(input);input.select();input.setSelectionRange?.(0,input.value.length);return document.execCommand("copy")===true}catch{return false}finally{input.remove()}}
function showCouponCopyFeedback(button){if(!button?.classList?.contains("couponCopyLarge"))return;const previous=couponCopyFeedbackTimers.get(button),token=(previous?.token||0)+1;if(previous?.timer)clearTimeout(previous.timer);button.classList.add("copied");const liveStatus=button.querySelector(".couponCopyLiveStatus");if(liveStatus)liveStatus.textContent="Copied to Clipboard";const timer=setTimeout(()=>{const active=couponCopyFeedbackTimers.get(button);if(active?.token!==token)return;couponCopyFeedbackTimers.delete(button);if(!button.isConnected)return;button.classList.remove("copied");const currentStatus=button.querySelector(".couponCopyLiveStatus");if(currentStatus)currentStatus.textContent=""},1800);couponCopyFeedbackTimers.set(button,{timer,token})}
function renderCouponDetail(c){const rewards=c.rewards||[],first=rewards[0],redeemed=couponIsRedeemed(c.code),rewardKey=couponCodeKey(c.code),expanded=couponState.expandedRewardsCode===rewardKey,rewardListId=`couponRewardList-${rewardKey||"selected"}`,codexMatched=rewards.some(r=>r.iconSource==="BDO Codex");couponEl.detail.innerHTML=`<button class="couponDetailClose">&times;</button><span class="couponDetailBadge">${c.isExpired?"EXPIRED":"AVAILABLE"} <i></i></span><h2>${couponEscape(c.code)} <button class="couponInlineCopy" data-copy-coupon="${couponEscape(c.code)}">&#9633;</button></h2><div class="couponDetailLead">Redeem this code in-game to claim your rewards.</div><div class="couponDetailStats"><div class="couponDetailStat"><span class="couponDetailStatIcon">${redeemed?"&#10003;":"&#9633;"}</span><div><label>REDEMPTION STATUS</label><strong class="${redeemed?"green":"muted"}">${redeemed?"Redeemed":"Not redeemed"}</strong><small>${redeemed?"You marked this coupon as used.":"Mark it redeemed after using it in-game."}</small></div></div><div class="couponDetailStat"><span class="couponDetailStatIcon">&#10003;</span><div><label>COUPON STATUS</label><strong class="green">${c.isExpired?"Expired":"Available"}</strong><small>${couponEscape(couponExpiryText(c.expiryUtc,c.isExpired,c.expiryText))}</small></div></div></div><section class="couponRewardPreview"><button class="couponRewardDisclosure" type="button" data-coupon-rewards-toggle="${couponEscape(c.code)}" aria-expanded="${expanded}" aria-controls="${couponEscape(rewardListId)}"><span class="couponRewardDisclosureMain"><small>REWARDS PREVIEW</small>${first?`<span class="couponRewardDisclosureReward"><img src="${couponEscape(first.icon)}" alt=""><span>${couponEscape(first.quantity)}x ${couponEscape(first.itemName)}</span></span>`:"<span>Reward details unavailable.</span>"}</span><strong class="couponRewardCount">${rewards.length} ${rewards.length===1?"item":"items"}</strong><span class="couponRewardDisclosureChevron" aria-hidden="true"></span></button><div class="couponRewardList" id="${couponEscape(rewardListId)}" ${expanded?"":"hidden"}>${couponRewardListHtml(rewards)}${codexMatched?'<button class="couponIconAttribution" type="button" data-open-url="https://bdocodex.com/">Item icons: BDO Codex &nearr;</button>':""}</div></section><button class="couponCopyLarge" type="button" data-copy-coupon="${couponEscape(c.code)}" aria-label="Copy coupon code ${couponEscape(c.code)}"><span class="couponCopyLargeDefault" aria-hidden="true">Copy Code</span><span class="couponCopyLargeSuccess" aria-hidden="true">Copied to Clipboard</span><span class="couponCopyLiveStatus" role="status" aria-live="polite" aria-atomic="true"></span></button><button class="couponRedeemOnline" data-open-url="https://payment.naeu.playblackdesert.com/en-us/Shop/Coupon/">Redeem Online &nbsp; &nearr;</button>`;}
async function initializeCoupons(){if(couponState.initialized)return;couponState.initialized=true;try{applyCouponDashboard(await bridgeCall("initializeCoupons"));startCouponAutoRefresh();setTimeout(()=>refreshCoupons({auto:true,silent:false}),1800);}catch(error){couponEl.message.textContent=error.message;}}
...
couponEl.statusAlert?.addEventListener("click",openAvailableCoupons);
...
couponEl.rows?.addEventListener("click",async event=>{const button=event.target.closest("[data-copy-coupon]");if(!button)return;const code=button.dataset.copyCoupon;if(await copyCouponCodeToClipboard(code))NotificationService.ShowSuccess(`Copied ${code}`);else NotificationService.ShowError("Could not copy the coupon code.");});
...
// Official September 3, 2026 Life Skill Lightstone rework. Keep the imported
// Adventurer's Guide snapshot above intact and layer the dated patch over it so
// future upstream refreshes can replace the baseline without losing provenance.
const LIGHTSTONE_PATCH_2026_09_03_SOURCE="Official NA/EU Patch Notes - September 3, 2026";
const LIGHTSTONE_PATCH_2026_09_03=Object.freeze({
  "Delotia":{effects:["Life EXP +17%"],lightstones:["Flora: Wildlife","Flora: Wildlife","Flora: Wildlife","Iridescent Lightstone"]},
  "Hand of Manos":{effects:["Life Skill Mastery +30"],lightstones:["Flora: Paradise","Flora: Paradise","Flora: Paradise","Iridescent Lightstone"]},
  "Fortress of Nature":{effects:["Life EXP +12%","Life Skill Mastery +20"],lightstones:["Flora: Wildlife","Flora: Paradise","Flora: Wildlife","Iridescent Lightstone"]},
  "Scurrying Weasel":{effects:["Gathering Item Drop Rate +5%","Gathering EXP +10%","Gathering Mastery +20","Gathering Speed +1"],lightstones:["Flora: Plains","Flora: Forest","Flora: Plains","Iridescent Lightstone"]},
  "Yawning Hedgehog":{effects:["Gathering Item Drop Rate +10%","Gathering EXP +10%","Gathering Mastery +20","Gathering Speed +1"],lightstones:["Flora: Plains","Flora: Forest","Flora: Wildlife","Iridescent Lightstone"]},
  "Choice & Focus: Gathering":{effects:["Gathering Mastery -500","Gathering EXP +35%"],lightstones:["Flora: Plains","Flora: Plains","Flora: Wildlife","Iridescent Lightstone"]},
  "Crouching Predator":{effects:["Hunting EXP +7%","Hunting Mastery +15"],lightstones:["Flora: Trap","Flora: Track","Flora: Trap","Flora: Trap"]},
  "Blink of an Eye":{effects:["Matchlock Reload Speed +10%","Hunting EXP +10%","Hunting Mastery +20"],lightstones:["Flora: Trap","Flora: Track","Flora: Track","Iridescent Lightstone"]},
  "Choice & Focus: Hunting":{effects:["Hunting Mastery -500","Hunting EXP +35%"],lightstones:["Flora: Trap","Flora: Trap","Flora: Wildlife","Iridescent Lightstone"]},
  "Nibbles":{effects:["Auto-fishing Time -15%","Fishing EXP +10%","Fishing Mastery +20","Fishing Speed +1"],lightstones:["Flora: Bite","Flora: Patience","Flora: Bite","Iridescent Lightstone"]},
  "Whaling":{effects:["Chance to Catch High-Quality Fish +6%","Fishing EXP +10%","Fishing Mastery +20","Fishing Speed +1"],lightstones:["Flora: Bite","Flora: Patience","Flora: Paradise","Iridescent Lightstone"]},
  "Sharp-eyed Seagull":{effects:["Chance to Catch Rare Fish +5%","Fishing EXP +10%","Fishing Mastery +20","Fishing Speed +1"],lightstones:["Flora: Bite","Flora: Patience","Flora: Wildlife","Iridescent Lightstone"]},
  "Choice & Focus: Fishing":{effects:["Fishing Mastery -500","Fishing EXP +35%"],lightstones:["Flora: Bite","Flora: Bite","Flora: Wildlife","Iridescent Lightstone"]},
  "Fundamentals of Cooking":{effects:["Cooking Time -2 sec","Cooking EXP +10%","Cooking Mastery +25"],lightstones:["Flora: Secret","Flora: Stir","Flora: Stir","Iridescent Lightstone"]},
  "Choice & Focus: Cooking":{effects:["Cooking Mastery -500","Cooking EXP +35%"],lightstones:["Flora: Secret","Flora: Secret","Flora: Wildlife","Iridescent Lightstone"]},
  "Choice & Focus: Training":{effects:["Training Mastery -500","Training EXP +35%"],lightstones:["Flora: Gallop","Flora: Gallop","Flora: Wildlife","Iridescent Lightstone"]},
  "A Fragment of a Star, a Spoonful of the Moon":{effects:["Alchemy Time -2 sec","Alchemy EXP +10%","Alchemy Mastery +25"],lightstones:["Flora: Time","Flora: Malleable","Flora: Malleable","Iridescent Lightstone"]},
  "Choice & Focus: Alchemy":{effects:["Alchemy Mastery -500","Alchemy EXP +35%"],lightstones:["Flora: Time","Flora: Time","Flora: Wildlife","Iridescent Lightstone"]},
  "Choice & Focus: Sailing":{effects:["Sailing Mastery -500","Sailing EXP +35%"],lightstones:["Flora: Uncharted","Flora: Uncharted","Flora: Wildlife","Iridescent Lightstone"]},
  "Clang! Clang!":{effects:["Processing Success Rate +20%","Processing EXP +10%","Processing Mastery +25"],lightstones:["Flora: Tool","Flora: Deft","Flora: Deft","Iridescent Lightstone"]},
  "Choice & Focus: Processing":{effects:["Processing Mastery -500","Processing EXP +35%"],lightstones:["Flora: Tool","Flora: Tool","Flora: Wildlife","Iridescent Lightstone"]}
});
const LIGHTSTONE_REMOVED_2026_09_03=Object.freeze(["Canine Tooth","Blacksmith's Blessing","Refreshing Dream"]);
function applyLightstonePatch20260903(){
  const removed=new Set(LIGHTSTONE_REMOVED_2026_09_03);
  for(let index=LIGHTSTONE_SETS.length-1;index>=0;index--){if(removed.has(LIGHTSTONE_SETS[index].name))LIGHTSTONE_SETS.splice(index,1)}
  LIGHTSTONE_SETS.forEach(set=>{
    const patch=LIGHTSTONE_PATCH_2026_09_03[set.name];
    if(!patch)return;
    set.effects=[...patch.effects];
    set.lightstones=[...patch.lightstones];
    set.source=LIGHTSTONE_PATCH_2026_09_03_SOURCE;
  });
}
applyLightstonePatch20260903();

const lightstoneState={ready:false,type:null,transition:null,search:"",category:"",amplified:false};
const lightstoneEl={chooser:document.getElementById("lightstoneChooser"),stage:document.getElementById("lightstoneStage"),title:document.getElementById("lightstoneStageTitle"),subtitle:document.getElementById("lightstoneStageSubtitle"),grid:document.getElementById("lightstoneSetGrid"),pill:document.getElementById("lightstoneModePill"),search:document.getElementById("lightstoneSearch"),category:document.getElementById("lightstoneCategoryFilter"),summary:document.getElementById("lightstoneSummary"),amplifiedControl:document.getElementById("lightstoneAmplifiedControl"),amplified:document.getElementById("lightstoneAmplifiedToggle")};
...
function lightstoneAmplifiedMode(){return lightstoneState.type==="combat"&&lightstoneState.amplified}
function renderLightstoneStone(stone){const amp=amplifiedLightstoneFor(stone),useAmp=lightstoneAmplifiedMode()&&amp,color=lightstoneColorForStone(stone),name=useAmp?amp.name:stone,ampLine=useAmp?`<span class="lightstoneAmpLine">${escapeHtml(amp.effect)}</span>`:"";return `<li class="lightstoneStone" style="--stone-color:${color}"><span class="lightstoneStoneName">${escapeHtml(name)}</span>${ampLine}</li>`}
...
function renderLightstoneCard(set){const effects=set.effects.map(effect=>`<li>${escapeHtml(effect)}</li>`).join("");const stones=set.lightstones.map(renderLightstoneStone).join("");const tags=[set.category,...lightstoneSetTags(set)].map(tag=>`<span class="lightstoneTag ${tag==="Amplified"?"lightstoneAmpTag":""}">${escapeHtml(tag)}</span>`).join("");return `<article class="lightstoneSetCard"><div class="lightstoneSetTop"><h3>${escapeHtml(set.name)}</h3><div class="lightstoneTagRow">${tags}</div></div><div class="lightstoneBlock"><strong>Set effect</strong><ul class="lightstoneLines">${effects}</ul></div><div class="lightstoneBlock"><strong>${lightstoneAmplifiedMode()?"Required lightstones - amplified where available":"Required lightstones"}</strong><ul class="lightstoneLines">${stones}</ul></div></article>`}
function renderLightstoneSets(){
  if(!lightstoneEl.stage||!lightstoneEl.grid)return;
  const type=lightstoneState.type,amplifiedAvailable=type==="combat",amplifiedMode=lightstoneAmplifiedMode();
  document.querySelectorAll("[data-lightstone-choice]").forEach(button=>button.classList.toggle("active",button.dataset.lightstoneChoice===type));
  lightstoneEl.chooser?.classList.toggle("hidden",Boolean(type));
  lightstoneEl.stage.classList.toggle("active",Boolean(type));
  lightstoneEl.pill.textContent=type?`${lightstoneTypeLabel(type)} selected`:"Select a category";
  if(lightstoneEl.amplifiedControl)lightstoneEl.amplifiedControl.hidden=!amplifiedAvailable;
  if(lightstoneEl.amplified){lightstoneEl.amplified.checked=lightstoneState.amplified;lightstoneEl.amplified.disabled=!amplifiedAvailable}
  if(lightstoneEl.subtitle){
    lightstoneEl.subtitle.hidden=type!=="lifeskill";
    lightstoneEl.subtitle.textContent=type==="lifeskill"?"Updated September 3, 2026 — Life Skill combinations now use only Flora or Iridescent Lightstones.":"";
  }
  if(!type){lightstoneEl.grid.innerHTML="";if(lightstoneEl.summary)lightstoneEl.summary.textContent="";return;}
  renderLightstoneCategoryOptions(type);
  const label=lightstoneTypeLabel(type),sets=getLightstoneSets(type);
  const filtered=sets.filter(set=>(!lightstoneState.category||set.category===lightstoneState.category||lightstoneSetTags(set).includes(lightstoneState.category))&&lightstoneMatches(set,lightstoneState.search));
  lightstoneEl.title.textContent=`${label} Lightstone Sets`;
  if(lightstoneEl.summary){const categoryText=lightstoneState.category?` in ${lightstoneState.category}`:"";const ampText=amplifiedMode?`<span>Amplified mode on</span>`:"";lightstoneEl.summary.innerHTML=`<span>Showing ${filtered.length} of ${sets.length} ${label.toLowerCase()} sets${categoryText}</span>${ampText}${lightstoneState.search?`<span>Search: ${escapeHtml(lightstoneState.search)}</span>`:""}`}
  lightstoneEl.grid.innerHTML=filtered.length?filtered.map(renderLightstoneCard).join(""):`<div class="lightstoneEmpty">No lightstone sets match the current filters.</div>`;
}
...
function serverTimeZoneLabel(date=new Date()){
  const offsetMinutes=Math.round(zonedOffsetMs(date,HOME_SERVER_TIME_ZONE)/60000);
  return offsetMinutes===60?"CET":offsetMinutes===120?"CEST":HOME_SERVER_TIME_ZONE_LABEL;
}
...
const NODE_WAR_HOUR=20;
const NODE_WAR_ALERT_MILESTONES=Object.freeze([0,5,15,30]);
const NODE_WAR_NOTIFICATION_MODES=Object.freeze(["off","tts","alarm"]);
const DEFAULT_NOTIFICATION_VOLUME_PERCENT=50;
const MAX_TTS_VOICE_ID_LENGTH=2048;
const RESET_TIMER_CONFIG = [
  ...
  { id:"nodewar", label:"Node War", kind:"nodewar", hour:NODE_WAR_HOUR, minute:0, detail:"Every day at 20:00 CET/CEST except Saturday" },
...
const RESET_TIMER_ICON_SVGS = Object.freeze({
  ...
  nodewar:'<svg viewBox="0 0 24 24"><path d="M12 3 4 7v5c0 5 3.4 8.4 8 9 4.6-.6 8-4 8-9V7l-8-4Z"></path><path d="m8.5 15 7-7"></path><path d="m9 8 7 7"></path></svg>',
...
const resetTimerEl={grid:document.getElementById("resetTimersGrid"),status:document.getElementById("resetTimersStatus"),localTime:document.getElementById("resetLocalTime"),nodeWarMode:document.getElementById("nodeWarNotificationMode"),nodeWarLead:document.getElementById("nodeWarLeadTime")};
const homeEl={
  head:document.getElementById("bossScheduleHead"),body:document.getElementById("bossScheduleBody"),nextBossTitle:document.getElementById("homeNextBossTitle"),nextBossTime:document.getElementById("homeNextBossTime"),nextBossSub:document.getElementById("homeNextBossSub"),dayNightCard:document.getElementById("homeDayNightCard"),dayNightValue:document.getElementById("homeDayNightValue"),dayNightSub:document.getElementById("homeDayNightSub"),dayNightProgress:document.getElementById("homeDayNightProgress"),guildBossValue:document.getElementById("homeGuildBossValue"),guildBossSub:document.getElementById("homeGuildBossSub"),guildBossDay:document.getElementById("homeGuildBossDay"),guildBossTime:document.getElementById("homeGuildBossTime"),guildBossSetSub:document.getElementById("homeGuildBossSetSub"),nextBossLine:document.getElementById("homeBossNextLine"),time12:document.getElementById("homeTime12"),time24:document.getElementById("homeTime24"),localTime:document.getElementById("homeLocalTime"),panel:document.getElementById("bossNotifyPanel"),collapse:document.getElementById("bossNotifyCollapse"),content:document.getElementById("bossNotifyContent"),master:document.getElementById("bossMasterNotifications"),lead:document.getElementById("bossLeadTime"),sound:document.getElementById("bossSoundEnabled"),tts:document.getElementById("bossTtsEnabled"),guildNotify:document.getElementById("guildBossNotifications"),volume:document.getElementById("bossNotificationVolume"),volumeValue:document.getElementById("bossNotificationVolumeValue"),voice:document.getElementById("bossTtsVoice"),addVoices:document.getElementById("bossAddVoices"),testTts:document.getElementById("bossTestTts"),testAlarm:document.getElementById("bossTestAlarm"),toggles:document.getElementById("bossToggleList"),footer:document.getElementById("bossNotifyFooter"),status:document.getElementById("homeScheduleStatus")
};
...
function normalizeNotificationVolumePercent(value){const numeric=Number(value);return Number.isFinite(numeric)?Math.max(0,Math.min(100,Math.round(numeric))):DEFAULT_NOTIFICATION_VOLUME_PERCENT}
function normalizeTtsVoiceId(value){return typeof value==="string"?value.trim().slice(0,MAX_TTS_VOICE_ID_LENGTH):""}
function normalizedNotificationAudioSettings(){const saved=readSetting("notificationAudioSettings",{});return {volumePercent:normalizeNotificationVolumePercent(saved.volumePercent),voiceId:normalizeTtsVoiceId(saved.voiceId)}}
function saveNotificationAudioSettings(settings){persistSetting("notificationAudioSettings",{volumePercent:normalizeNotificationVolumePercent(settings?.volumePercent),voiceId:normalizeTtsVoiceId(settings?.voiceId)})}
function normalizedHomeSettings(){const saved=readSetting("homeSettings",{});return {timeFormat:saved.timeFormat==="24"?"24":"12",showLocalTime:saved.showLocalTime===true,masterNotifications:saved.masterNotifications!==false,leadMinutes:[0,5,10,15,30].includes(Number(saved.leadMinutes))?Number(saved.leadMinutes):15,soundEnabled:saved.soundEnabled!==false,ttsEnabled:saved.ttsEnabled===true,guildBossNotifications:saved.guildBossNotifications===true,notificationsCollapsed:saved.notificationsCollapsed===true,bosses:{...defaultBossSelection(),...(saved.bosses||{})},notified:saved.notified&&typeof saved.notified==="object"?saved.notified:{}}}
...
function nextNodeWarOccurrence(now=new Date(),includeRecentlyStarted=false){const p=zonedParts(now,HOME_SERVER_TIME_ZONE),earliest=now.getTime()-(includeRecentlyStarted?HOME_SPAWNING_NOW_GRACE_MS:0);for(let dayOffset=0;dayOffset<8;dayOffset++){const candidate=zonedTimeToDate(HOME_SERVER_TIME_ZONE,p.year,p.month,p.day+dayOffset,NODE_WAR_HOUR,0);if(zonedParts(candidate,HOME_SERVER_TIME_ZONE).weekday==="Sat")continue;if((includeRecentlyStarted&&candidate.getTime()>=earliest)||(!includeRecentlyStarted&&candidate>now))return candidate}return null}
function resetTimerTarget(config,now=new Date()){if(config.kind==="nodewar")return nextNodeWarOccurrence(now);return config.kind==="hours"?nextServerReset(config.hours,now):nextDailyServerReset(config.hour,config.minute||0,now)}
function normalizedResetSettings(){const saved=readSetting("resetTimerSettings",{}),mode=NODE_WAR_NOTIFICATION_MODES.includes(saved.nodeWarNotificationMode)?saved.nodeWarNotificationMode:"off",lead=Number(saved.nodeWarLeadMinutes);return {showLocalTime:saved.showLocalTime===true,timeFormat:(saved.timeFormat==="24"||normalizedHomeSettings().timeFormat==="24")?"24":"12",nodeWarNotificationMode:mode,nodeWarLeadMinutes:NODE_WAR_ALERT_MILESTONES.includes(lead)?lead:15,nodeWarNotified:saved.nodeWarNotified&&typeof saved.nodeWarNotified==="object"?saved.nodeWarNotified:{}}}
...
function renderResetTimers(settings=normalizedResetSettings(),now=new Date()){
  if(!resetTimerEl.grid)return;
  if(resetTimerEl.localTime)resetTimerEl.localTime.checked=settings.showLocalTime;
  if(resetTimerEl.nodeWarMode)resetTimerEl.nodeWarMode.value=settings.nodeWarNotificationMode;
  if(resetTimerEl.nodeWarLead){resetTimerEl.nodeWarLead.value=String(settings.nodeWarLeadMinutes);resetTimerEl.nodeWarLead.disabled=settings.nodeWarNotificationMode==="off"}
  ...
  for(const config of RESET_TIMER_CONFIG){
    ...
    if(next){const day=config.kind==="nodewar"?`${settings.showLocalTime?localDayName(target):zonedParts(target,HOME_SERVER_TIME_ZONE).weekday} `:"";next.textContent=`${config.kind==="nodewar"?"Next battle":"Next reset"}: ${day}${resetTimerServerLabel(target,settings)} - ${HOME_TIMER_CONFIG.region}`}
  }
  if(resetTimerEl.status)resetTimerEl.status.textContent=`Timers loaded - Last updated ${now.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}`;
}
function initializeResetTimers(){renderResetTimers(normalizedResetSettings())}

/* WEEKLY_PLANNER_START */
const WEEKLY_DAY_MS=86400000;
const WEEKLY_STATE_KEY="weekliesSettings";
const WEEKLY_RESET_POLICIES=Object.freeze({
  thursday:{label:"Thursday reset",short:"Thu 00:00 UTC",weekday:4,reminders:true},
  sunday:{label:"Sunday reset",short:"Sun 00:00 UTC",weekday:0,reminders:true},
  monday:{label:"Guild reset",short:"Mon 00:00 UTC",weekday:1,reminders:true},
  fishing:{label:"Fishing contest rollover",short:"Mon 00:20 UTC",weekday:1,hour:0,minute:20,reminders:true},
  scheduled:{label:"Scheduled activity",short:"Schedule in Timers",weekday:null,cycleWeekday:4,reminders:false},
  daily:{label:"Daily reset",short:"Daily 00:00 UTC",weekday:null,reminders:false},
  rolling120:{label:"Rolling respawn",short:"About 120 hours",weekday:null,reminders:false}
});
const WEEKLY_TASK_ACCENTS=Object.freeze(["#2dd4bf","#f97316","#a78bfa","#22d3ee","#fb7185","#facc15","#60a5fa","#34d399","#f472b6","#c084fc","#38bdf8","#fbbf24","#818cf8","#4ade80","#fb923c","#e879f9","#5eead4","#f87171","#93c5fd","#d8b4fe","#67e8f9","#fdba74","#86efac","#f9a8d4","#a5b4fc","#fde047","#6ee7b7","#7dd3fc","#fda4af","#c4b5fd","#fcd34d","#bef264"]);
const WEEKLY_CATALOG=Object.freeze([
  {id:"black-shrine-donghae",name:"Black Shrine: Donghae (Solo)",category:"Core combat",description:"Use your five Family reward clears across the solo Shrine bosses.",reset:"sunday",recommended:true,meta:"5 clears"},
  {id:"black-shrine-hwanghae",name:"Black Shrine: Hwanghae (Party)",category:"Core combat",description:"Complete the separate five-clear party Shrine allowance.",reset:"sunday",recommended:true,meta:"5 clears"},
  {id:"pit-of-the-undying",name:"Pit of the Undying",category:"Core combat",description:"Accept Chorpy's weekly and defeat up to four arena opponents.",reset:"thursday",recommended:true,meta:"Weekly quest"},
  {id:"altar-of-blood",name:"Altar of Blood",category:"Core combat",description:"Set your best stage for this week's highest-stage reward.",reset:"sunday",recommended:true,meta:"Weekly high score"},
  {id:"edania-boss-subjugation",name:"Edania Boss Subjugation",category:"Core combat",description:"Choose and complete one weekly boss across Outer or Inner Edania.",reset:"thursday",recommended:false,meta:"Choose 1 · Endgame"},
  {id:"garmoth-weekly-loot",name:"Garmoth Weekly Loot",category:"Core combat",description:"Claim the three sequential weekly Garmoth reward quests.",reset:"thursday",recommended:true,meta:"3 rewards"},
  {id:"dark-rifts-sweep",name:"Dark Rifts Sweep",category:"Core combat",description:"A personal sweep timer. Rift bosses return individually about 120 hours after each defeat.",reset:"rolling120",recommended:false,meta:"Rolling tracker"},

  {id:"atoraxxion-vahmalkea",name:"Atoraxxion: Vahmalkea",category:"Atoraxxion",description:"Complete the biome and collect its weekly reward chest.",reset:"thursday",recommended:true,meta:"Weekly chest"},
  {id:"atoraxxion-sycrakea",name:"Atoraxxion: Sycrakea",category:"Atoraxxion",description:"Complete the biome and collect its weekly reward chest.",reset:"thursday",recommended:true,meta:"Weekly chest"},
  {id:"atoraxxion-yolunakea",name:"Atoraxxion: Yolunakea",category:"Atoraxxion",description:"Complete the biome and collect its weekly reward chest.",reset:"thursday",recommended:true,meta:"Weekly chest"},
  {id:"atoraxxion-orzekea",name:"Atoraxxion: Orzekea",category:"Atoraxxion",description:"Complete the biome and collect its weekly reward chest.",reset:"thursday",recommended:true,meta:"Weekly chest"},
  {id:"atoraxxion-final-gladios",name:"Atoraxxion: Final Gladios",category:"Atoraxxion",description:"Finish the separate solo Final Gladios weekly challenge.",reset:"thursday",recommended:false,meta:"Solo weekly"},

  {id:"potion-dragons-fang",name:"Dragon's Fang",category:"Potion progression",description:"Complete the weekly quest for five Dragon's Fang pity pieces.",reset:"thursday",recommended:false,meta:"5 pity pieces"},
  {id:"potion-tranquil-tinniolum",name:"Tranquil Tinniolum",category:"Potion progression",description:"Complete the weekly quest for five Tranquil Tinniolum pity pieces.",reset:"thursday",recommended:false,meta:"5 pity pieces"},
  {id:"potion-blood-wolfs-oath",name:"Blood Wolf's Oath",category:"Potion progression",description:"Complete the weekly quest for five Blood Wolf's Oath pity pieces.",reset:"thursday",recommended:false,meta:"5 pity pieces"},
  {id:"potion-valtarras-memory",name:"Valtarra's Memory",category:"Potion progression",description:"Complete the weekly quest for five Valtarra's Memory pity pieces.",reset:"thursday",recommended:false,meta:"5 pity pieces"},
  {id:"potion-katzvariaks-venom",name:"Katzvariak's Venom",category:"Potion progression",description:"Complete the weekly quest for five Katzvariak's Venom pity pieces.",reset:"thursday",recommended:false,meta:"5 pity pieces"},
  {id:"potion-narcs-solace",name:"Narc's Solace",category:"Potion progression",description:"Complete the weekly quest for five Narc's Solace pity pieces.",reset:"thursday",recommended:false,meta:"5 pity pieces"},

  {id:"season-stronger-tuvala",name:"Stronger Tuvala Gear",category:"Progression",description:"Choose one eligible monster-zone objective on a Season character.",reset:"thursday",recommended:false,meta:"Season weekly"},
  {id:"jetina-pen-weekly",name:"Jetina PEN Progression",category:"Progression",description:"Track your current Jetina weekly progression objective.",reset:"thursday",recommended:false,meta:"Goal dependent"},
  {id:"garmoth-heart-weekly",name:"Garmoth Heart Progression",category:"Progression",description:"Complete the weekly step for the guaranteed Garmoth Heart path.",reset:"thursday",recommended:false,meta:"Goal dependent"},
  {id:"mythical-feather-weekly",name:"Mythical Feather Weekly",category:"Progression",description:"Complete your chosen weekly Mythical Feather exchange quest.",reset:"thursday",recommended:false,meta:"Dream horse"},
  {id:"lara-bread-weekly",name:"Lara's Bread Weekly",category:"Progression",description:"Finish Lara's weekly bread quest for its progression reward.",reset:"thursday",recommended:false,meta:"Weekly quest"},
  {id:"altar-of-training",name:"Altar of Training",category:"Progression",description:"Complete the weekly Altar of Training objective when it is part of your route.",reset:"thursday",recommended:false,meta:"Weekly quest"},
  {id:"sycraia-upper-weekly",name:"Sycraia Upper Weekly",category:"Progression",description:"Complete the weekly objective in the upper Sycraia Underwater Ruins.",reset:"thursday",recommended:false,meta:"Monster-zone weekly"},
  {id:"sycraia-lower-weekly",name:"Sycraia Lower Weekly",category:"Progression",description:"Complete the weekly objective in the lower Sycraia Underwater Ruins.",reset:"thursday",recommended:false,meta:"Monster-zone weekly"},

  {id:"weekly-fishing-contest",name:"Weekly Fishing Contest",category:"Life skills",description:"Submit your best catch during the Monday-to-Saturday contest window and collect the result reward.",reset:"fishing",recommended:false,meta:"Special weekly schedule"},

  {id:"ocean-candidum",name:"Candidum Hunter",category:"Ocean & guild",description:"Defeat one Candidum for the Old Moon Guild ocean weekly.",reset:"thursday",recommended:false,meta:"Ocean weekly"},
  {id:"ocean-nineshark",name:"Nineshark Hunter",category:"Ocean & guild",description:"Defeat one Nineshark for the Old Moon Guild ocean weekly.",reset:"thursday",recommended:false,meta:"Ocean weekly"},
  {id:"ocean-black-rust",name:"Black Rust Hunter",category:"Ocean & guild",description:"Defeat one Black Rust for the Old Moon Guild ocean weekly.",reset:"thursday",recommended:false,meta:"Ocean weekly"},
  {id:"sailing-dailies",name:"Sailing Dailies (Sailies)",category:"Ocean & guild",description:"Track the ordinary sailing circuit separately; these quests reset daily, not weekly.",reset:"daily",recommended:false,meta:"Daily · optional"},
  {id:"guild-boss-raid",name:"Guild Boss Raid",category:"Ocean & guild",description:"Personal attendance tracker for your guild's weekly boss summons.",reset:"monday",recommended:false,meta:"Guild-owned"}
].map((task,index)=>Object.freeze({...task,accent:WEEKLY_TASK_ACCENTS[index%WEEKLY_TASK_ACCENTS.length]})));
const WEEKLY_CATALOG_BY_ID=new Map(WEEKLY_CATALOG.map(task=>[task.id,task]));
const WEEKLY_CATEGORY_ORDER=[...new Set(WEEKLY_CATALOG.map(task=>task.category))];
const WEEKLY_CATEGORY_ICONS=Object.freeze({
  "Core combat":'<svg viewBox="0 0 24 24"><path d="m14.5 4.5 5 5M13 6l5 5M4 20l5.5-1.5L19 9l-4-4-9.5 9.5Z"></path><path d="m4 20 4-4"></path></svg>',
  Atoraxxion:'<svg viewBox="0 0 24 24"><path d="m12 2 8 5v10l-8 5-8-5V7Z"></path><path d="m12 7 4 2.5V15l-4 2.5L8 15V9.5Z"></path></svg>',
  "Potion progression":'<svg viewBox="0 0 24 24"><path d="M9 2h6M10 2v5l-5 8a4.5 4.5 0 0 0 3.8 7h6.4a4.5 4.5 0 0 0 3.8-7l-5-8V2"></path><path d="M7 15h10"></path></svg>',
  Progression:'<svg viewBox="0 0 24 24"><path d="M4 19V9M10 19V5M16 19V2M22 19H2"></path><path d="m4 9 6-4 6-3"></path></svg>',
  "Life skills":'<svg viewBox="0 0 24 24"><path d="M5 3v12a5 5 0 0 0 10 0V8"></path><path d="M5 6h7a3 3 0 0 1 3 3v6M15 15c2.8 0 5-1.8 5-4"></path><circle cx="20" cy="9" r="2"></circle></svg>',
  "Ocean & guild":'<svg viewBox="0 0 24 24"><path d="M12 3v18M5 8h14M3 14h18"></path><path d="M5 14a7 7 0 0 0 14 0M12 3a3 3 0 0 1 3 3h-6a3 3 0 0 1 3-3Z"></path></svg>'
});
const weekliesEl={
  onboarding:document.getElementById("weekliesOnboarding"),dashboard:document.getElementById("weekliesDashboard"),dashboardActions:document.getElementById("weekliesDashboardActions"),onboardingSearch:document.getElementById("weekliesOnboardingSearch"),onboardingCategories:document.getElementById("weekliesOnboardingCategories"),onboardingList:document.getElementById("weekliesOnboardingList"),onboardingCount:document.getElementById("weekliesOnboardingCount"),onboardingHint:document.getElementById("weekliesOnboardingHint"),selectRecommended:document.getElementById("weekliesSelectRecommended"),finish:document.getElementById("weekliesFinishOnboarding"),completedCount:document.getElementById("weekliesCompletedCount"),completedLabel:document.getElementById("weekliesCompletedLabel"),remainingCount:document.getElementById("weekliesRemainingCount"),nextReset:document.getElementById("weekliesNextReset"),nextResetLabel:document.getElementById("weekliesNextResetLabel"),progressPercent:document.getElementById("weekliesProgressPercent"),progressBar:document.getElementById("weekliesProgressBar"),dashboardList:document.getElementById("weekliesDashboardList"),liveStatus:document.getElementById("weekliesLiveStatus"),allDone:document.getElementById("weekliesAllDone"),openSettings:document.getElementById("weekliesOpenSettings"),overlay:document.getElementById("weekliesSettingsOverlay"),dialog:document.querySelector(".weekliesSettingsDialog"),closeSettings:document.getElementById("weekliesCloseSettings"),cancelSettings:document.getElementById("weekliesCancelSettings"),saveSettings:document.getElementById("weekliesSaveSettings"),settingsSearch:document.getElementById("weekliesSettingsSearch"),settingsList:document.getElementById("weekliesSettingsList"),settingsCount:document.getElementById("weekliesSettingsCount"),reminderEnabled:document.getElementById("weekliesReminderEnabled"),reminderDays:document.getElementById("weekliesReminderDays"),statusAlert:document.getElementById("weekliesStatusAlert")
};
const weekliesRuntime={initialized:false,hydrated:false,writeGeneration:0,writeQueue:Promise.resolve(),onboardingSelected:new Set(),onboardingQuery:"",onboardingCategory:"all",settingsSelected:new Set(),settingsQuery:"",settingsReturnFocus:null,overlayTimer:null,lastRenderCycle:"",badgeCount:-1,inFlight:new Set()};

function weeklyStringMap(value,maxEntries=4096){const result={};if(!value||typeof value!=="object"||Array.isArray(value))return result;for(const [rawKey,rawValue] of Object.entries(value).slice(0,maxEntries)){const key=String(rawKey||"").trim().slice(0,256);if(key&&typeof rawValue==="string"&&rawValue.length<=256)result[key]=rawValue}return result}
function weeklyBooleanMap(value,maxEntries=4096){const result={};if(!value||typeof value!=="object"||Array.isArray(value))return result;for(const [rawKey,rawValue] of Object.entries(value).slice(0,maxEntries)){const key=String(rawKey||"").trim().slice(0,256);if(key&&rawValue===true)result[key]=true}return result}
function normalizeWeeklyState(saved={}){const selectedIds=[...new Set((Array.isArray(saved?.selectedIds)?saved.selectedIds:[]).map(value=>String(value||"").trim()).filter(id=>WEEKLY_CATALOG_BY_ID.has(id)))],rawRevision=Number(saved?.revision),revision=Number.isSafeInteger(rawRevision)&&rawRevision>=0?rawRevision:0;const reminder=Number(saved?.reminderDays);return{schemaVersion:1,revision,onboardingComplete:saved?.onboardingComplete===true,selectedIds,reminderDays:[1,2,3].includes(reminder)?reminder:1,notificationsEnabled:saved?.notificationsEnabled===true,doneById:weeklyStringMap(saved?.doneById),notified:weeklyBooleanMap(saved?.notified)}}
let weeklySettings=normalizeWeeklyState(readSetting(WEEKLY_STATE_KEY,{}));
function weeklyStatePayload(){return{schemaVersion:1,revision:weeklySettings.revision,onboardingComplete:weeklySettings.onboardingComplete,selectedIds:[...weeklySettings.selectedIds],reminderDays:weeklySettings.reminderDays,notificationsEnabled:weeklySettings.notificationsEnabled,doneById:{...weeklySettings.doneById},notified:{...weeklySettings.notified}}}
function weeklyQueueNativeSave(){if(!weekliesRuntime.hydrated){weekliesRuntime.writeGeneration++;return Promise.resolve(null)}const payload=weeklyStatePayload(),generation=++weekliesRuntime.writeGeneration;weekliesRuntime.writeQueue=weekliesRuntime.writeQueue.catch(()=>null).then(()=>bridgeCall("saveWeeklyPlannerState",payload)).then(result=>{if(generation===weekliesRuntime.writeGeneration&&result?.state)weeklySettings=normalizeWeeklyState(result.state);return result}).catch(error=>{console.warn("[Weeklies] update-safe save failed",error);return null});return weekliesRuntime.writeQueue}
function persistWeeklyState({native=true,render=true}={}){weeklySettings=normalizeWeeklyState(weeklySettings);weeklySettings.revision=Math.min(Number.MAX_SAFE_INTEGER,Math.max(Date.now(),weeklySettings.revision+1));persistSetting(WEEKLY_STATE_KEY,weeklyStatePayload());flushSetting(WEEKLY_STATE_KEY);if(native)weeklyQueueNativeSave();if(render)renderWeeklies();updateWeeklyStatusAlert()}
async function hydrateWeeklyState(){const generation=weekliesRuntime.writeGeneration,localState=normalizeWeeklyState(weeklySettings);let nativeStateExists=false,syncNative=false;try{const result=await bridgeCall("initializeWeeklyPlanner",{});nativeStateExists=result?.stateExists===true;if(generation===weekliesRuntime.writeGeneration&&nativeStateExists&&result?.state){const nativeState=normalizeWeeklyState(result.state);weeklySettings=nativeState.revision>=localState.revision?nativeState:localState;syncNative=localState.revision>nativeState.revision;persistSetting(WEEKLY_STATE_KEY,weeklyStatePayload());flushSetting(WEEKLY_STATE_KEY)}}catch(error){console.warn("[Weeklies] durable state unavailable",error)}finally{weekliesRuntime.hydrated=true;weekliesRuntime.onboardingSelected=new Set(weeklySettings.selectedIds);renderWeeklies();updateWeeklyStatusAlert();if(!nativeStateExists||syncNative||generation!==weekliesRuntime.writeGeneration)weeklyQueueNativeSave()}}
function nextWeeklyUtcWeekday(weekday,now=new Date(),hour=0,minute=0){const current=now.getUTCDay(),days=(weekday-current+7)%7;let target=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),now.getUTCDate()+days,hour,minute,0));if(target<=now)target=new Date(target.getTime()+7*WEEKLY_DAY_MS);return target}
function weeklyTaskTarget(task,now=new Date()){if(!task)return null;if(task.reset==="daily"){let target=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),now.getUTCDate()+1));return target}if(task.reset==="rolling120"){const completed=Date.parse(weeklySettings.doneById[task.id]||"");return Number.isFinite(completed)?new Date(completed+120*3600000):null}const policy=WEEKLY_RESET_POLICIES[task.reset];return Number.isInteger(policy?.weekday)?nextWeeklyUtcWeekday(policy.weekday,now,policy.hour||0,policy.minute||0):null}
function weeklyTaskPeriodKey(task,now=new Date()){if(task?.reset==="rolling120")return now.toISOString();const policy=WEEKLY_RESET_POLICIES[task?.reset];if(Number.isInteger(policy?.cycleWeekday))return nextWeeklyUtcWeekday(policy.cycleWeekday,now).toISOString();return weeklyTaskTarget(task,now)?.toISOString()||"manual"}
function weeklyTaskIsDone(task,now=new Date()){const value=weeklySettings.doneById[task.id];if(!value)return false;if(task.reset==="rolling120"){const completed=Date.parse(value);return Number.isFinite(completed)&&completed+120*3600000>now.getTime()}return value===weeklyTaskPeriodKey(task,now)}
function weeklyTaskRenderSignature(task,now=new Date()){if(task?.reset==="rolling120")return weeklyTaskIsDone(task,now)?`active:${weeklySettings.doneById[task.id]}`:"ready";return weeklyTaskPeriodKey(task,now)}
function weeklyAnnounce(message){if(!weekliesEl.liveStatus)return;weekliesEl.liveStatus.textContent="";requestAnimationFrame(()=>{if(weekliesEl.liveStatus)weekliesEl.liveStatus.textContent=message})}
function weeklyMotionDelay(milliseconds){return document.body.dataset.motion==="reduced"||window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches?0:milliseconds}
function weeklySelectedTasks(){return weeklySettings.selectedIds.map(id=>WEEKLY_CATALOG_BY_ID.get(id)).filter(Boolean)}
function weeklyPolicyLabel(task){return WEEKLY_RESET_POLICIES[task.reset]?.short||"Manual"}
function weeklyFormatCountdown(ms){const remaining=Math.max(0,Number(ms)||0);if(remaining>WEEKLY_DAY_MS){const days=Math.floor(remaining/WEEKLY_DAY_MS);return`${days} day${days===1?"":"s"} remaining`}const seconds=Math.floor(remaining/1000),hours=Math.floor(seconds/3600),minutes=Math.floor(seconds%3600/60),rest=seconds%60;return`${String(hours).padStart(2,"0")}:${String(minutes).padStart(2,"0")}:${String(rest).padStart(2,"0")}`}
function weeklyCountdownState(ms){const remaining=Number(ms);return Number.isFinite(remaining)&&remaining>=0&&remaining<=WEEKLY_DAY_MS?"urgent":"standard"}
function weeklyTaskCountdownState(task,now=new Date()){if(task?.reset==="rolling120"&&!weeklyTaskIsDone(task,now))return"urgent";const target=weeklyTaskTarget(task,now);return target?weeklyCountdownState(target-now):"standard"}
function weeklyTaskCountdown(task,now=new Date()){const target=weeklyTaskTarget(task,now);if(task.reset==="rolling120"&&!weeklyTaskIsDone(task,now))return"Ready now";return target?weeklyFormatCountdown(target-now):"No fixed reset"}
function weeklyResetDescription(task,now=new Date()){if(task.reset==="rolling120"){const target=weeklyTaskTarget(task,now);return weeklyTaskIsDone(task,now)&&target?`Personal window ends ${target.toLocaleString([], {weekday:"short",hour:"2-digit",minute:"2-digit"})}`:"Each boss uses its own rolling respawn"}const target=weeklyTaskTarget(task,now),local=target?.toLocaleString([],{weekday:"short",hour:"2-digit",minute:"2-digit"});return target?`${weeklyPolicyLabel(task)} · ${local} local`:weeklyPolicyLabel(task)}
function weeklyTaskSearchText(task){return`${task.name} ${task.category} ${task.description} ${task.meta} ${weeklyPolicyLabel(task)}`.toLowerCase()}
function weeklyPickerMarkup(task,selected,context){const icon=WEEKLY_CATEGORY_ICONS[task.category]||WEEKLY_CATEGORY_ICONS.Progression;return`<button class="weeklyPickerCard ${selected?"selected":""}" type="button" data-weekly-pick="${task.id}" data-weekly-context="${context}" aria-pressed="${selected}" style="--weekly-task-accent:${task.accent}"><span class="weeklyPickerCheck" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="m5 12 4 4L19 6"></path></svg></span><span class="weeklyPickerIcon" aria-hidden="true">${icon}</span><span class="weeklyPickerCopy"><span class="weeklyPickerTitle"><strong>${escapeHtml(task.name)}</strong>${task.recommended?'<i>Essential</i>':""}</span><span>${escapeHtml(task.description)}</span><small><b>${escapeHtml(weeklyPolicyLabel(task))}</b><em>${escapeHtml(task.meta)}</em></small></span></button>`}
function renderWeeklyPicker(target,selected,query="",category="all",context="onboarding"){if(!target)return;const normalizedQuery=String(query||"").trim().toLowerCase(),visible=WEEKLY_CATALOG.filter(task=>(category==="all"||task.category===category)&&(!normalizedQuery||weeklyTaskSearchText(task).includes(normalizedQuery)));target.innerHTML=visible.length?WEEKLY_CATEGORY_ORDER.map(group=>{const tasks=visible.filter(task=>task.category===group);return tasks.length?`<section class="weekliesPickerGroup"><header><span>${WEEKLY_CATEGORY_ICONS[group]||""}</span><div><h2>${escapeHtml(group)}</h2><small>${tasks.length} option${tasks.length===1?"":"s"}</small></div></header><div>${tasks.map(task=>weeklyPickerMarkup(task,selected.has(task.id),context)).join("")}</div></section>`:""}).join(""):'<div class="weekliesPickerEmpty"><strong>No matching weeklies</strong><span>Try a boss, region, dungeon, or progression name.</span></div>'}
function renderWeeklyCategoryChips(){if(!weekliesEl.onboardingCategories)return;weekliesEl.onboardingCategories.innerHTML=["all",...WEEKLY_CATEGORY_ORDER].map(category=>`<button type="button" data-weekly-category="${escapeHtml(category)}" class="${weekliesRuntime.onboardingCategory===category?"active":""}">${category==="all"?"All":escapeHtml(category)}</button>`).join("")}
function renderWeekliesOnboarding(){renderWeeklyCategoryChips();renderWeeklyPicker(weekliesEl.onboardingList,weekliesRuntime.onboardingSelected,weekliesRuntime.onboardingQuery,weekliesRuntime.onboardingCategory,"onboarding");const count=weekliesRuntime.onboardingSelected.size;if(weekliesEl.onboardingCount)weekliesEl.onboardingCount.textContent=String(count);if(weekliesEl.onboardingHint)weekliesEl.onboardingHint.textContent=count?`${count} activit${count===1?"y":"ies"} will be added to your route.`:"Choose at least one activity to continue.";if(weekliesEl.finish)weekliesEl.finish.disabled=count===0}
function weeklyCardMarkup(task,now){const done=weeklyTaskIsDone(task,now),icon=WEEKLY_CATEGORY_ICONS[task.category]||WEEKLY_CATEGORY_ICONS.Progression;return`<article class="weeklyTaskCard ${done?"isDone":""}" data-weekly-task="${task.id}" data-weekly-done-state="${done}" style="--weekly-task-accent:${task.accent}"><button class="weeklyDoneToggle" type="button" data-weekly-toggle-done="${task.id}" aria-pressed="${done}" aria-label="Mark ${escapeHtml(task.name)} ${done?"not done":"done"}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m5 12 4 4L19 6"></path></svg></button><span class="weeklyTaskIcon" aria-hidden="true">${icon}</span><div class="weeklyTaskCopy"><h3>${escapeHtml(task.name)}</h3><p>${escapeHtml(task.description)}</p></div><div class="weeklyTaskClock"><strong data-weekly-countdown data-weekly-countdown-state="${weeklyTaskCountdownState(task,now)}">${escapeHtml(weeklyTaskCountdown(task,now))}</strong><span data-weekly-reset-description>${escapeHtml(weeklyResetDescription(task,now))}</span></div></article>`}
function weeklyNextSelectedReset(tasks,now){return tasks.map(task=>({task,target:weeklyTaskTarget(task,now)})).filter(entry=>entry.target&&entry.target>now).sort((a,b)=>a.target-b.target)[0]||null}
function renderWeeklyDashboard(now=new Date()){
  const tasks=weeklySelectedTasks(),done=tasks.filter(task=>weeklyTaskIsDone(task,now)),remaining=tasks.length-done.length,percent=tasks.length?Math.round(done.length/tasks.length*100):0;
  if(weekliesEl.completedCount)weekliesEl.completedCount.textContent=String(done.length);
  if(weekliesEl.completedLabel)weekliesEl.completedLabel.textContent=`of ${tasks.length} this cycle`;
  if(weekliesEl.remainingCount)weekliesEl.remainingCount.textContent=String(remaining);
  if(weekliesEl.progressPercent)weekliesEl.progressPercent.textContent=`${percent}%`;
  if(weekliesEl.progressBar)weekliesEl.progressBar.style.width=`${percent}%`;
  if(weekliesEl.allDone){weekliesEl.allDone.disabled=remaining===0;weekliesEl.allDone.classList.toggle("complete",tasks.length>0&&remaining===0);weekliesEl.allDone.setAttribute("aria-pressed",String(tasks.length>0&&remaining===0));const label=weekliesEl.allDone.querySelector("span");if(label)label.textContent=tasks.length&&remaining===0?"All complete":"All done"}
  const next=weeklyNextSelectedReset(tasks.filter(task=>!weeklyTaskIsDone(task,now)),now)||weeklyNextSelectedReset(tasks,now);
...
function closeWeekliesSettings(){if(!weekliesEl.overlay||weekliesEl.overlay.hidden)return;weekliesEl.overlay.classList.remove("isOpen");document.body.classList.remove("weekliesDialogOpen");clearTimeout(weekliesRuntime.overlayTimer);weekliesRuntime.overlayTimer=setTimeout(()=>{if(weekliesEl.overlay)weekliesEl.overlay.hidden=true;weekliesRuntime.settingsReturnFocus?.focus?.();weekliesRuntime.settingsReturnFocus=null},weeklyMotionDelay(190))}
function saveWeekliesSettings(){weeklySettings.selectedIds=WEEKLY_CATALOG.filter(task=>weekliesRuntime.settingsSelected.has(task.id)).map(task=>task.id);weeklySettings.notificationsEnabled=weekliesEl.reminderEnabled?.checked===true;weeklySettings.reminderDays=[1,2,3].includes(Number(weekliesEl.reminderDays?.value))?Number(weekliesEl.reminderDays.value):1;weeklySettings.onboardingComplete=true;persistWeeklyState();closeWeekliesSettings();weeklyAnnounce(`Weekly settings saved with ${weeklySettings.selectedIds.length} selected activities.`);NotificationService.ShowSuccess("Your activities and reminder window were saved.","Weekly settings")}
function weeklyOverlayKeydown(event){if(weekliesEl.overlay?.hidden)return;if(event.key==="Escape"){event.preventDefault();closeWeekliesSettings();return}if(event.key!=="Tab"||!weekliesEl.dialog)return;const focusable=[...weekliesEl.dialog.querySelectorAll('button:not([disabled]),input:not([disabled]),select:not([disabled]),[tabindex]:not([tabindex="-1"])')].filter(element=>!element.hidden);if(!focusable.length)return;const first=focusable[0],last=focusable.at(-1);if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}}
function weeklyActiveReminderGroups(now=new Date()){if(!weeklySettings.onboardingComplete||!weeklySettings.notificationsEnabled)return[];const groups=new Map();for(const task of weeklySelectedTasks()){if(weeklyTaskIsDone(task,now)||WEEKLY_RESET_POLICIES[task.reset]?.reminders!==true)continue;const target=weeklyTaskTarget(task,now);if(!target)continue;const delta=target-now;if(delta<=0||delta>weeklySettings.reminderDays*WEEKLY_DAY_MS)continue;const key=target.toISOString(),group=groups.get(key)||{target,tasks:[]};group.tasks.push(task);groups.set(key,group)}return[...groups.values()].sort((a,b)=>a.target-b.target)}
function updateWeeklyStatusAlert(now=new Date()){const groups=weeklyActiveReminderGroups(now),count=groups.reduce((sum,group)=>sum+group.tasks.length,0);if(weekliesEl.statusAlert){weekliesEl.statusAlert.textContent=count?`${count} ${count===1?"weekly":"weeklies"} due`:"";weekliesEl.statusAlert.classList.toggle("show",count>0)}if(weekliesRuntime.badgeCount!==count){weekliesRuntime.badgeCount=count;bridgeCall("setWeekliesBadgeCount",{count}).catch(()=>{})}return count}
function openWeekliesView(){const button=document.querySelector('[data-app-view="weekliesView"]');if(!button)return false;activateAppView(button);setTimeout(()=>document.getElementById("weekliesView")?.scrollIntoView({block:"start"}),220);return true}
function weeklyPruneNotified(now=new Date()){const cutoff=now.getTime()-21*WEEKLY_DAY_MS;for(const key of Object.keys(weeklySettings.notified)){const match=key.match(/weekly\|([^|]+)\|\d+$/),time=match?Date.parse(match[1]):NaN;if(Number.isFinite(time)&&time<cutoff)delete weeklySettings.notified[key]}}
async function checkWeeklyNotifications(now=new Date()){if(!weekliesRuntime.hydrated)return false;const groups=weeklyActiveReminderGroups(now);updateWeeklyStatusAlert(now);if(!groups.length)return false;let delivered=false;for(const group of groups){const days=Math.max(1,Math.ceil((group.target-now)/WEEKLY_DAY_MS));if(days>weeklySettings.reminderDays)continue;const key=`weekly|${group.target.toISOString()}|${days}`;if(weeklySettings.notified[key]||weekliesRuntime.inFlight.has(key))continue;weekliesRuntime.inFlight.add(key);try{const count=group.tasks.length,title=days===1?"Weeklies reset tomorrow":`Weeklies reset in ${days} days`,names=group.tasks.slice(0,3).map(task=>task.name),extra=count>3?` and ${count-3} more`:"",message=`${count} unfinished: ${names.join(", ")}${extra}.`;await bridgeCall("showDesktopNotification",{title,message});bridgeCall("flashTaskbarAttention",{}).catch(()=>{});NotificationService.ShowInfo(message,title);weeklySettings.notified[key]=true;weeklyPruneNotified(now);persistWeeklyState({render:false});delivered=true}catch(error){console.warn("[Weeklies] reminder delivery failed",error)}finally{weekliesRuntime.inFlight.delete(key)}}return delivered}
function tickWeeklies(now=new Date()){
  if(!weeklySettings.onboardingComplete)return;
  const tasks=weeklySelectedTasks(),cycle=tasks.map(task=>`${task.id}:${weeklyTaskIsDone(task,now)}:${weeklyTaskRenderSignature(task,now)}`).join("|");
  if(cycle!==weekliesRuntime.lastRenderCycle){renderWeeklyDashboard(now);updateWeeklyStatusAlert(now);return}
  for(const task of tasks){
    const card=weekliesEl.dashboardList?.querySelector(`[data-weekly-task="${task.id}"]`);
    if(!card)continue;
    const countdown=card.querySelector("[data-weekly-countdown]"),description=card.querySelector("[data-weekly-reset-description]");
    if(countdown){countdown.textContent=weeklyTaskCountdown(task,now);countdown.dataset.weeklyCountdownState=weeklyTaskCountdownState(task,now)}
    if(description)description.textContent=weeklyResetDescription(task,now)
  }
  const next=weeklyNextSelectedReset(tasks.filter(task=>!weeklyTaskIsDone(task,now)),now)||weeklyNextSelectedReset(tasks,now);
  if(weekliesEl.nextReset){weekliesEl.nextReset.textContent=next?weeklyFormatCountdown(next.target-now):"All clear";weekliesEl.nextReset.dataset.weeklyCountdownState=next?weeklyCountdownState(next.target-now):"standard"}
  updateWeeklyStatusAlert(now)
}
function initializeWeeklies(){if(weekliesRuntime.initialized)return;weekliesRuntime.initialized=true;weekliesRuntime.onboardingSelected=new Set(weeklySettings.selectedIds);weekliesEl.onboardingList?.addEventListener("click",event=>{const button=event.target.closest("[data-weekly-pick]");if(button)weeklySetPickerSelection("onboarding",button.dataset.weeklyPick)});weekliesEl.settingsList?.addEventListener("click",event=>{const button=event.target.closest("[data-weekly-pick]");if(button)weeklySetPickerSelection("settings",button.dataset.weeklyPick)});weekliesEl.onboardingCategories?.addEventListener("click",event=>{const button=event.target.closest("[data-weekly-category]");if(!button)return;weekliesRuntime.onboardingCategory=button.dataset.weeklyCategory||"all";renderWeekliesOnboarding()});weekliesEl.onboardingSearch?.addEventListener("input",()=>{weekliesRuntime.onboardingQuery=weekliesEl.onboardingSearch.value;renderWeekliesOnboarding()});weekliesEl.settingsSearch?.addEventListener("input",()=>{weekliesRuntime.settingsQuery=weekliesEl.settingsSearch.value;renderWeekliesSettingsPicker()});weekliesEl.selectRecommended?.addEventListener("click",()=>{weekliesRuntime.onboardingSelected=new Set(WEEKLY_CATALOG.filter(task=>task.recommended).map(task=>task.id));renderWeekliesOnboarding()});weekliesEl.finish?.addEventListener("click",()=>{if(!weekliesRuntime.onboardingSelected.size)return;weeklySettings.selectedIds=WEEKLY_CATALOG.filter(task=>weekliesRuntime.onboardingSelected.has(task.id)).map(task=>task.id);weeklySettings.onboardingComplete=true;persistWeeklyState({render:false});weeklyTransitionToDashboard();weeklyAnnounce(`Weekly route created with ${weeklySettings.selectedIds.length} activities.`);NotificationService.ShowSuccess("Your weekly route is ready.","Weeklies")});weekliesEl.dashboardList?.addEventListener("click",event=>{const toggle=event.target.closest("[data-weekly-toggle-done]");if(toggle){toggleWeeklyDone(toggle.dataset.weeklyToggleDone);return}if(event.target.closest("[data-weekly-open-settings]"))openWeekliesSettings()});weekliesEl.allDone?.addEventListener("click",markAllWeekliesDone);weekliesEl.openSettings?.addEventListener("click",openWeekliesSettings);weekliesEl.closeSettings?.addEventListener("click",closeWeekliesSettings);weekliesEl.cancelSettings?.addEventListener("click",closeWeekliesSettings);weekliesEl.saveSettings?.addEventListener("click",saveWeekliesSettings);weekliesEl.reminderEnabled?.addEventListener("change",()=>{if(weekliesEl.reminderDays)weekliesEl.reminderDays.disabled=!weekliesEl.reminderEnabled.checked});weekliesEl.overlay?.addEventListener("click",event=>{if(event.target===weekliesEl.overlay)closeWeekliesSettings()});document.addEventListener("keydown",weeklyOverlayKeydown);weekliesEl.statusAlert?.addEventListener("click",openWeekliesView);renderWeeklies();hydrateWeeklyState()}
/* WEEKLY_PLANNER_END */
...
function applyBossNotifyCollapse(collapsed){const isCollapsed=collapsed===true;if(homeEl.panel)homeEl.panel.classList.toggle("isCollapsed",isCollapsed);if(homeEl.content){homeEl.content.hidden=false;homeEl.content.inert=isCollapsed;homeEl.content.setAttribute("aria-hidden",String(isCollapsed))}if(homeEl.collapse){homeEl.collapse.setAttribute("aria-expanded",String(!isCollapsed));const label=isCollapsed?"Expand boss notifications":"Collapse boss notifications";homeEl.collapse.title=label;const accessibleLabel=homeEl.collapse.querySelector(".srOnly");if(accessibleLabel)accessibleLabel.textContent=label}}
function applyNotificationAudioSettings(settings){const volume=normalizeNotificationVolumePercent(settings?.volumePercent),voiceId=normalizeTtsVoiceId(settings?.voiceId);if(homeEl.volume){homeEl.volume.value=String(volume);homeEl.volume.style.setProperty("--boss-volume",`${volume}%`);homeEl.volume.setAttribute("aria-valuetext",volume===0?"Muted":`${volume} percent`)}if(homeEl.volumeValue)homeEl.volumeValue.textContent=volume===0?"Muted":`${volume}%`;if(homeEl.voice&&Array.from(homeEl.voice.options).some(option=>option.value===voiceId))homeEl.voice.value=voiceId}
function applyHomeSettings(settings){if(homeEl.time12)homeEl.time12.classList.toggle("active",settings.timeFormat==="12");if(homeEl.time24)homeEl.time24.classList.toggle("active",settings.timeFormat==="24");if(homeEl.localTime)homeEl.localTime.checked=settings.showLocalTime;applyBossNotifyCollapse(settings.notificationsCollapsed);if(homeEl.master)homeEl.master.checked=settings.masterNotifications;if(homeEl.lead)homeEl.lead.value=String(settings.leadMinutes);if(homeEl.sound){homeEl.sound.checked=settings.soundEnabled;homeEl.sound.disabled=false;homeEl.sound.closest(".bossSettingRow")?.classList.remove("disabledBoss")}if(homeEl.tts)homeEl.tts.checked=settings.ttsEnabled;if(homeEl.guildNotify)homeEl.guildNotify.checked=settings.guildBossNotifications;renderBossSchedule(settings);renderBossToggles(settings);updateHomeTimers(settings)}
...
function nodeWarAlertStage(settings,delta,keyBase){const requestedLead=Number(settings.nodeWarLeadMinutes),lead=NODE_WAR_ALERT_MILESTONES.includes(requestedLead)?requestedLead:15;if(delta < -HOME_SPAWNING_NOW_GRACE_MS||delta>lead*60000)return null;const stage=NODE_WAR_ALERT_MILESTONES.find(minutes=>minutes<=lead&&delta<=minutes*60000),notified=settings.nodeWarNotified||{};return stage!==undefined&&!notified[`${keyBase}|${stage}`]?stage:null}
function pruneNodeWarNotifications(settings){const cutoff=Date.now()-14*86400000;for(const key of Object.keys(settings.nodeWarNotified||{})){const timestamp=notificationKeyDate(key);if(!Number.isNaN(timestamp)&&timestamp<cutoff)delete settings.nodeWarNotified[key]}}
async function sendNodeWarAlert(title,message,spokenText,settings){const audio=normalizedNotificationAudioSettings(),channel=settings.nodeWarNotificationMode==="alarm"?{name:"Alarm.mp3",promise:bridgeCall("playAlarmSound",{volumePercent:audio.volumePercent})}:settings.nodeWarNotificationMode==="tts"?{name:"text to speech",promise:bridgeCall("speakText",{text:spokenText,volumePercent:audio.volumePercent,voiceId:audio.voiceId})}:null;if(!channel)return false;try{await channel.promise;NotificationService.ShowInfo(message,title);return true}catch(error){console.warn("[NodeWarNotify] Alert channel failure",{channel:channel.name,error});NotificationService.ShowError(`Could not deliver the Node War ${channel.name} alert.`,"Node War alert failed");return false}}
async function persistDeliveredNodeWarAlert(key,settings,deliver){if(nodeWarAlertInFlight.has(key))return false;nodeWarAlertInFlight.add(key);try{const delivered=await deliver();if(delivered){const latest=normalizedResetSettings(),nodeWarNotified={...(latest.nodeWarNotified||{}),...(settings.nodeWarNotified||{}),[key]:true},merged={...latest,nodeWarNotified};settings.nodeWarNotified=nodeWarNotified;pruneNodeWarNotifications(merged);saveResetSettings(merged)}return delivered}finally{nodeWarAlertInFlight.delete(key)}}
async function checkNodeWarNotifications(settings,now=new Date()){if(settings.nodeWarNotificationMode==="off")return false;const target=nextNodeWarOccurrence(now,true);if(!target)return false;const delta=target-now,keyBase=`node-war|${target.toISOString()}`,stage=nodeWarAlertStage(settings,delta,keyBase);if(stage===null)return false;const key=`${keyBase}|${stage}`,starting=stage===0,warningMinutes=starting?0:Math.max(1,Math.ceil(delta/60000)),warningText=alertLeadText(warningMinutes),title=starting?"Node War - Starting Now":"Node War starting soon",message=starting?`${resetTimerServerLabel(target,settings)} - Node War is starting now - ${HOME_TIMER_CONFIG.region}`:`${resetTimerServerLabel(target,settings)} - Node War starts in ${warningText} - ${HOME_TIMER_CONFIG.region}`,spoken=starting?"Nodewar starting now.":`Nodewar starting in ${warningText}.`;return persistDeliveredNodeWarAlert(key,settings,()=>sendNodeWarAlert(title,message,spoken,settings))}
...
function runBackgroundNotifications(){if(homeAlertRunPromise)return homeAlertRunPromise;const settings=normalizedHomeSettings(),resetSettings=normalizedResetSettings(),now=new Date(),next=nextAlertableBossSpawn(settings,now);homeAlertRunPromise=Promise.all([checkBossNotifications(settings,now,next),checkGuildBossNotifications(settings,now),checkNodeWarNotifications(resetSettings,now),checkWeeklyNotifications(now)]).catch(error=>{console.warn("[BackgroundNotify] Alert check failed",error)}).finally(()=>{homeAlertRunPromise=null});return homeAlertRunPromise}
...
function populateEnglishTtsVoices(result){if(!homeEl.voice)return;const source=Array.isArray(result?.voices)?result.voices:[],voiceMap=new Map();for(const candidate of source){const id=normalizeTtsVoiceId(candidate?.id),name=String(candidate?.name||"").trim();if(id&&name&&!voiceMap.has(id.toLowerCase()))voiceMap.set(id.toLowerCase(),{id,name})}const voices=Array.from(voiceMap.values()).sort((a,b)=>a.name.localeCompare(b.name,undefined,{sensitivity:"base"})),automatic=document.createElement("option"),defaultVoice=voices.find(voice=>voice.id===normalizeTtsVoiceId(result?.defaultVoiceId));automatic.value="";automatic.textContent=defaultVoice?`Automatic (${defaultVoice.name})`:"Automatic (recommended)";homeEl.voice.replaceChildren(automatic);for(const voice of voices){const option=document.createElement("option");option.value=voice.id;option.textContent=voice.name;homeEl.voice.append(option)}const audio=normalizedNotificationAudioSettings(),hasSavedVoice=voices.some(voice=>voice.id===audio.voiceId);if(!hasSavedVoice&&audio.voiceId){audio.voiceId="";saveNotificationAudioSettings(audio)}homeEl.voice.value=hasSavedVoice?audio.voiceId:"";homeEl.voice.disabled=voices.length===0;if(!voices.length)automatic.textContent="No English Windows voices installed"}
function initializeTtsVoiceSelector(){if(!homeEl.voice)return Promise.resolve();if(ttsVoiceLoadPromise)return ttsVoiceLoadPromise;homeEl.voice.disabled=true;ttsVoiceLoadPromise=bridgeCall("getEnglishTtsVoices",{}).then(populateEnglishTtsVoices).catch(error=>{console.warn("[TTS] Could not enumerate installed English voices",error);const option=document.createElement("option");option.value="";option.textContent="Automatic (voice list unavailable)";homeEl.voice.replaceChildren(option);homeEl.voice.disabled=true});return ttsVoiceLoadPromise}
function initializeHomeDashboard(){const settings=normalizedHomeSettings();applyHomeSettings(settings);applyNotificationAudioSettings(normalizedNotificationAudioSettings());initializeTtsVoiceSelector();initializeBossScheduleSync()}
...
homeEl.collapse?.addEventListener("click",()=>{const settings=normalizedHomeSettings();settings.notificationsCollapsed=!settings.notificationsCollapsed;saveHomeSettings(settings);applyBossNotifyCollapse(settings.notificationsCollapsed)});
homeEl.volume?.addEventListener("input",()=>{const audio=normalizedNotificationAudioSettings();audio.volumePercent=normalizeNotificationVolumePercent(homeEl.volume.value);saveNotificationAudioSettings(audio);applyNotificationAudioSettings(audio)});
homeEl.volume?.addEventListener("change",()=>{const audio=normalizedNotificationAudioSettings(),message=audio.volumePercent===0?"Notification audio muted.":`Notification volume set to ${audio.volumePercent}%.`;setBossAlertTestStatus(message,"success");NotificationService.ShowInfo(message,"Audio settings saved")});
homeEl.voice?.addEventListener("change",()=>{const audio=normalizedNotificationAudioSettings();audio.voiceId=normalizeTtsVoiceId(homeEl.voice.value);saveNotificationAudioSettings(audio);const selected=homeEl.voice.selectedOptions[0]?.textContent||"Automatic";NotificationService.ShowInfo(`TTS voice: ${selected}.`,"Audio settings saved")});
homeEl.addVoices?.addEventListener("click",async()=>{if(homeEl.addVoices.disabled)return;homeEl.addVoices.disabled=true;try{await bridgeCall("openSpeechSettings",{});const message="Windows Speech settings opened. Installed English voices will appear after restarting Black Spirit Hub.";setBossAlertTestStatus(message,"success");NotificationService.ShowInfo(message,"Add TTS voices")}catch(error){NotificationService.ShowError(error.message||"Could not open Windows Speech settings.","Add TTS voices")}finally{homeEl.addVoices.disabled=false}});
...
function bossTestTtsText(settings=normalizedHomeSettings(),next=nextAlertableBossSpawn(settings)){if(!next)return"Black Spirit Hub text to speech test.";return settings.leadMinutes===0?`${spokenBossList(next.bosses)} spawning now.`:`${spokenBossList(next.bosses)} spawning in ${alertLeadText(settings.leadMinutes)}.`}
function runBossTtsTest(){const text=bossTestTtsText(),audio=normalizedNotificationAudioSettings();return runBossAlertTest(homeEl.testTts,"Speaking",()=>bridgeCall("speakText",{text,volumePercent:audio.volumePercent,voiceId:audio.voiceId}),result=>`English TTS played successfully${result?.voiceName?` with ${result.voiceName}`:""} at ${result?.volumePercent??audio.volumePercent}%: ${text}`)}
function runBossAlarmTest(){const audio=normalizedNotificationAudioSettings();return runBossAlertTest(homeEl.testAlarm,"Playing",()=>bridgeCall("playAlarmSound",{volumePercent:audio.volumePercent}),result=>`Alarm.mp3 is playing at ${result?.volumePercent??audio.volumePercent}%${result?.durationMilliseconds?` (${(result.durationMilliseconds/1000).toFixed(1)} seconds)`:``}.`)}
homeEl.testTts?.addEventListener("click",runBossTtsTest);
homeEl.testAlarm?.addEventListener("click",runBossAlarmTest);
...
resetTimerEl.localTime?.addEventListener("change",()=>{const settings=normalizedResetSettings();settings.showLocalTime=resetTimerEl.localTime.checked;saveResetSettings(settings);renderResetTimers(settings);NotificationService.ShowInfo(`Timers now show ${settings.showLocalTime?"local PC time":"EU server time"}.`,"Timer settings saved")});
resetTimerEl.nodeWarMode?.addEventListener("change",()=>{const settings=normalizedResetSettings();settings.nodeWarNotificationMode=NODE_WAR_NOTIFICATION_MODES.includes(resetTimerEl.nodeWarMode.value)?resetTimerEl.nodeWarMode.value:"off";saveResetSettings(settings);renderResetTimers(settings);const channel=settings.nodeWarNotificationMode==="tts"?"English TTS":settings.nodeWarNotificationMode==="alarm"?"Alarm.mp3":"off";NotificationService.ShowInfo(`Node War notifications: ${channel}.`,"Timer settings saved")});
resetTimerEl.nodeWarLead?.addEventListener("change",()=>{const settings=normalizedResetSettings(),lead=Number(resetTimerEl.nodeWarLead.value);settings.nodeWarLeadMinutes=NODE_WAR_ALERT_MILESTONES.includes(lead)?lead:15;saveResetSettings(settings);renderResetTimers(settings);const milestones=NODE_WAR_ALERT_MILESTONES.filter(minutes=>minutes<=settings.nodeWarLeadMinutes).sort((a,b)=>b-a).map(minutes=>minutes===0?"Nodewar Starting":`${minutes} minutes before`);NotificationService.ShowInfo(`Node War alerts: ${milestones.join(", ")}.`,"Timer settings saved")});
...
function recipeBookDerivedFishSubstitutionDefinition(payload,items){
  const members=new Map(),freshTier={1:"fresh-common",2:"fresh-high-quality",3:"fresh-rare"};
  for(const recipe of payload.recipes){
    if(!recipeBookIsPlainObject(recipe)||recipeBookTypeKey(recipe.type)!=="DRY"||!Array.isArray(recipe.inputs)||recipe.inputs.length!==1||!recipeBookIsPlainObject(recipe.inputs[0])||recipe.inputs[0].count!==1)continue;
    const input=recipe.inputs[0],freshId=String(input.itemId??"").trim(),driedId=String(recipe.outputId??"").trim(),fresh=items[freshId],dried=items[driedId],tier=freshTier[fresh?.grade];
    if(!tier||!fresh||!dried||(input.enhancement!==undefined&&input.enhancement!==0)||(recipe.outputEnhancement!==undefined&&recipe.outputEnhancement!==0)||!/(?:^|\n)- (?:Common|High-quality|Rare) Fish(?:\n|$)/iu.test(fresh.description)||!/(?:^|\n)- Usage:\s*Fried Fish(?:,|\n|$)/iu.test(fresh.description)||!dried.name.startsWith("Dried "))continue;
    members.set(freshId,[freshId,fresh.name,2,2,tier]);members.set(driedId,[driedId,dried.name,1,1,"dried"]);
  }
  if(!members.has("8201")||members.size<3)return null;
  const ordered=[...members.values()].sort((left,right)=>Number(left[0])-Number(right[0]));
  return recipeBookSubstitutionGroupDefinition("fish","Fish Group",["COOK"],8201,items["8201"].icon,ordered,true);
}
...
function recipeBookPrepareData(payload){
  ...
  const derivedFishDefinition=recipeBookDerivedFishSubstitutionDefinition(payload,items),substitutionDefinitions=Object.freeze(derivedFishDefinition?[...RECIPE_BOOK_SUBSTITUTION_GROUP_DEFINITIONS,derivedFishDefinition]:[...RECIPE_BOOK_SUBSTITUTION_GROUP_DEFINITIONS]);
  const substitutionDefinitionIds=new Set(),substitutionDefinitionMembers=new Set();
  for(const definition of substitutionDefinitions){
  ...
  const recipeIds=new Set(),recipes=payload.recipes.map((rawRecipe,index)=>{
    ...
    const hasOutputQuantity=rawRecipe.outputQuantityMin!==undefined||rawRecipe.outputQuantityMax!==undefined;
    recipeBookAssert(!hasOutputQuantity||(Number.isSafeInteger(rawRecipe.outputQuantityMin)&&rawRecipe.outputQuantityMin>0),`${path}.outputQuantityMin must be a positive whole number`);
    recipeBookAssert(!hasOutputQuantity||(Number.isSafeInteger(rawRecipe.outputQuantityMax)&&rawRecipe.outputQuantityMax>=rawRecipe.outputQuantityMin),`${path}.outputQuantityMax must be a whole number no smaller than outputQuantityMin`);
    recipeBookAssert(rawRecipe.outputQuantityNote===undefined||typeof rawRecipe.outputQuantityNote==="string",`${path}.outputQuantityNote must be a string`);
    recipeBookAssert(rawRecipe.officialSource===undefined||/^https:\/\//.test(rawRecipe.officialSource),`${path}.officialSource must be an HTTPS URL`);
    ...
    const output=items[outputId],station=(rawRecipe.station||"").trim(),recipeType=recipeBookTypeKey(type),genericAlternativeSearch=substitutionDefinitions.filter(definition=>definition.recipeTypes.includes(recipeType)&&inputs.some(input=>input.key===recipeBookIngredientKey(definition.representativeItemId,0))).flatMap(definition=>definition.members.map(member=>items[member.itemId]?.search||recipeBookSearchNorm(member.name)));
    return Object.freeze({
      ...
      ...(hasOutputQuantity?{outputQuantityMin:rawRecipe.outputQuantityMin,outputQuantityMax:rawRecipe.outputQuantityMax,outputQuantityNote:(rawRecipe.outputQuantityNote||"").trim()}:{}),
      ...(rawRecipe.officialSource?{officialSource:rawRecipe.officialSource}:{}),
  ...
  for(const group of Object.values(substitutionGroupLookup)){
    ...
    const representativeFactor=group.members.find(member=>member.key===group.representativeKey).factor,applicableRecipes=recipes.filter(recipe=>group.recipeTypes.includes(recipeBookTypeKey(recipe.type))&&recipe.inputs.some(input=>input.key===group.representativeKey)),applicableRecipeIds=new Set(applicableRecipes.map(recipe=>recipe.id));
    for(const member of group.members){
      ...
      for(const recipe of applicableRecipes){
        ...
        const equivalentCount=recipe.inputs.filter(input=>input.key===group.representativeKey).reduce((total,input)=>total+input.count,0)*representativeFactor;
...
const RECIPE_BOOK_OCR_RELEVANT_ICON_MIN_SCORE=.70,RECIPE_BOOK_OCR_RELEVANT_CLASS_MARGIN=.01,RECIPE_BOOK_OCR_EXACT_ICON_MIN_SCORE=.82,RECIPE_BOOK_OCR_EXACT_ICON_MARGIN=.08;
...
function recipeBookOcrSanitizeResult(value){
  ...
  for(const raw of rawSlots){
    ...
    for(const rawCandidate of Array.isArray(raw.iconCandidates)?raw.iconCandidates.slice(0,12):[]){
      if(!recipeBookIsPlainObject(rawCandidate))continue;const icon=recipeBookOcrCanonicalIcon(rawCandidate.icon),score=Number(rawCandidate.score),materialEligible=typeof rawCandidate.materialEligible==="boolean"?rawCandidate.materialEligible:null;
      if(!icon||icons.has(icon)||!Number.isFinite(score)||score<0||score>1)continue;icons.add(icon);iconCandidates.push(Object.freeze({icon,score,materialEligible}));
    }
    iconCandidates.sort((left,right)=>right.score-left.score||left.icon.localeCompare(right.icon));
    const quantityText=String(raw.quantityText??"").trim().slice(0,40),parsed=recipeBookOcrParseQuantity(quantityText),rawQuantity=raw.quantityValue,quantityValue=typeof rawQuantity==="number"&&Number.isSafeInteger(rawQuantity)&&rawQuantity>=1&&rawQuantity<=RECIPE_BOOK_OCR_MAX_QUANTITY?rawQuantity:null,quantityAssumedOne=Boolean(raw.quantityAssumedOne),rawSuggestedQuantity=raw.quantitySuggestedValue,quantitySuggestedValue=quantityValue===null&&!quantityAssumedOne&&parsed.valid&&typeof rawSuggestedQuantity==="number"&&Number.isSafeInteger(rawSuggestedQuantity)&&rawSuggestedQuantity>=1&&rawSuggestedQuantity<=RECIPE_BOOK_OCR_MAX_QUANTITY&&parsed.value===rawSuggestedQuantity?rawSuggestedQuantity:null,quantityConfidence=Number(raw.quantityConfidence),safeQuantityConfidence=Number.isFinite(quantityConfidence)&&quantityConfidence>=0&&quantityConfidence<=1?quantityConfidence:0,rawBorderGrade=raw.borderGrade,borderGrade=typeof rawBorderGrade==="number"&&Number.isInteger(rawBorderGrade)&&rawBorderGrade>=0&&rawBorderGrade<=2?rawBorderGrade:null,rawBorderGradeConfidence=raw.borderGradeConfidence,borderGradeConfidence=typeof rawBorderGradeConfidence==="number"&&Number.isFinite(rawBorderGradeConfidence)&&rawBorderGradeConfidence>=0&&rawBorderGradeConfidence<=1?rawBorderGradeConfidence:0,iconMaterialEligible=typeof raw.iconMaterialEligible==="boolean"?raw.iconMaterialEligible:null,rawIconMaterialConfidence=Number(raw.iconMaterialConfidence),iconMaterialConfidence=Number.isFinite(rawIconMaterialConfidence)&&rawIconMaterialConfidence>=0&&rawIconMaterialConfidence<=1?rawIconMaterialConfidence:0,rawIconMaterialMargin=Number(raw.iconMaterialMargin),iconMaterialMargin=Number.isFinite(rawIconMaterialMargin)&&rawIconMaterialMargin>=0&&rawIconMaterialMargin<=1?rawIconMaterialMargin:0;
    ids.add(id);slots.push(Object.freeze({id,row,column,box:Object.freeze({x,y,width:boxWidth,height:boxHeight}),iconCandidates:Object.freeze(iconCandidates),iconMaterialEligible,iconMaterialConfidence,iconMaterialMargin,quantityText,quantityValue,quantitySuggestedValue,quantityApproximate:Boolean(raw.quantityApproximate)||parsed.approximate,quantityConfidence:safeQuantityConfidence,quantityAssumedOne,borderGrade,borderGradeConfidence}));
  }
...
function recipeBookOcrPartitionReviewRows(analysis,data){
  if(!analysis||!data?.resourceItems||!data?.resourceLookup||!data?.items)return{rows:[],ignoredRows:[]};
  ...
  const eligibleItemIds=new Set(data.resourceItems.map(resource=>String(resource.itemId))),iconClasses=new Map();
  for(const [itemId,item] of Object.entries(data.items)){const icon=recipeBookOcrCanonicalIcon(item?.icon);if(!icon)continue;const classification=iconClasses.get(icon)||{eligible:false,negative:false};classification[eligibleItemIds.has(String(itemId))?"eligible":"negative"]=true;iconClasses.set(icon,classification)}
  for(const icon of iconIndex.keys()){const classification=iconClasses.get(icon)||{eligible:false,negative:false};classification.eligible=true;iconClasses.set(icon,classification)}
  const rows=[],ignoredRows=[];
  for(const slot of analysis.slots){
    const classifyCandidate=candidate=>candidate.materialEligible===true?{eligible:true,negative:false}:candidate.materialEligible===false?{eligible:false,negative:true}:iconClasses.get(candidate.icon),pureEligible=slot.iconCandidates.filter(candidate=>{const classification=classifyCandidate(candidate);return classification?.eligible&&!classification.negative}),pureNegative=slot.iconCandidates.filter(candidate=>{const classification=classifyCandidate(candidate);return classification?.negative&&!classification.eligible}),mixed=slot.iconCandidates.filter(candidate=>{const classification=classifyCandidate(candidate);return classification?.eligible&&classification.negative}),bestEligible=pureEligible[0]||null,bestNegative=pureNegative[0]||null,bestMixed=mixed[0]||null,bestUsable=bestEligible||bestMixed||slot.iconCandidates.find(candidate=>candidate.materialEligible!==false&&iconIndex.has(candidate.icon))||null,eligibleScore=bestEligible?.score||0,negativeScore=bestNegative?.score||0,mixedScore=bestMixed?.score||0,oppositionScore=Math.max(negativeScore,mixedScore),nativeNegative=slot.iconMaterialEligible===false,nativeMaterial=slot.iconMaterialEligible===true,relevance=nativeNegative?"negative":nativeMaterial&&bestUsable?"eligible":bestEligible&&eligibleScore>=RECIPE_BOOK_OCR_RELEVANT_ICON_MIN_SCORE&&eligibleScore-oppositionScore>=RECIPE_BOOK_OCR_RELEVANT_CLASS_MARGIN?"eligible":"uncertain",matchedIcon=relevance==="eligible"?(bestUsable||bestEligible):bestUsable,nextIcon=matchedIcon?slot.iconCandidates.find(candidate=>candidate!==matchedIcon&&candidate.materialEligible!==false)||null:null,topResources=matchedIcon?(iconIndex.get(matchedIcon.icon)||[]):[],gap=matchedIcon?matchedIcon.score-(nextIcon?.score||0):0,qualityFamily=recipeBookOcrQualityFamilyResources(topResources,data),trustedBorderGrade=Number.isInteger(slot.borderGrade)&&slot.borderGrade>=0&&slot.borderGrade<=2&&slot.borderGradeConfidence>=RECIPE_BOOK_OCR_BORDER_GRADE_CONFIDENCE&&qualityFamily.length>1,borderGradeMatches=trustedBorderGrade?qualityFamily.filter(resource=>data.items[resource.itemId]?.grade===slot.borderGrade):[],borderGradeResolved=trustedBorderGrade&&borderGradeMatches.length===1,borderGradeConflict=trustedBorderGrade&&!borderGradeResolved,resolvedKey=borderGradeResolved?borderGradeMatches[0].key:"",materialExactResources=trustedBorderGrade?borderGradeMatches:topResources,optionMap=new Map();
    const sharedMeatOrBloodIcon=topResources.length>1&&topResources.some(resource=>{const member=data.substitutionMemberByKey?.[resource.key],group=member&&data.substitutionGroupLookup?.[member.groupId];return Boolean(group?.sharedIcon&&/^(?:meat|blood)-/.test(group.id))});
    for(const iconMatch of slot.iconCandidates){if(iconMatch.materialEligible===false)continue;for(const resource of iconIndex.get(iconMatch.icon)||[]){const current=optionMap.get(resource.key);if(!current||iconMatch.score>current.score){const item=data.items[resource.itemId],enhancement=Number(resource.enhancement)||0;optionMap.set(resource.key,Object.freeze({key:resource.key,itemId:String(resource.itemId),enhancement,name:`${enhancement?`+${enhancement} `:""}${item?.name||"Unknown item"}`,score:iconMatch.score,uses:Number(resource.uses)||0}))}}}
    ...
    const unresolvedSharedMeatOrBloodIcon=sharedMeatOrBloodIcon&&materialExactResources.length!==1,defaultOption=relevance==="negative"||borderGradeConflict||unresolvedSharedMeatOrBloodIcon?null:borderGradeResolved?optionMap.get(resolvedKey):options[0]||null;
    const readQuantity=!slot.quantityAssumedOne&&Number.isSafeInteger(slot.quantityValue)?slot.quantityValue:!slot.quantityAssumedOne&&Number.isSafeInteger(slot.quantitySuggestedValue)?slot.quantitySuggestedValue:null,iconExact=Boolean(relevance==="eligible"&&matchedIcon&&matchedIcon.score>=RECIPE_BOOK_OCR_EXACT_ICON_MIN_SCORE&&gap>=RECIPE_BOOK_OCR_EXACT_ICON_MARGIN&&materialExactResources.length===1&&!unresolvedSharedMeatOrBloodIcon),quantityExact=Number.isSafeInteger(slot.quantityValue)&&slot.quantityValue>=1&&slot.quantityValue<=RECIPE_BOOK_OCR_MAX_QUANTITY&&slot.quantityConfidence>=.78&&!slot.quantityApproximate&&!slot.quantityAssumedOne,ready=iconExact&&quantityExact;
    const reasons=[];if(relevance==="negative")reasons.push("The icon strongly matched a known item that is not used as a Recipe Book input");else if(relevance==="uncertain"&&bestMixed&&mixedScore>=eligibleScore-RECIPE_BOOK_OCR_RELEVANT_CLASS_MARGIN)reasons.push("This artwork is shared by usable and finished or non-input items; review the suggested material before import");else if(relevance==="uncertain"&&bestEligible&&eligibleScore<RECIPE_BOOK_OCR_RELEVANT_ICON_MIN_SCORE)reasons.push("The suggested Recipe Book material needs review before import");else if(relevance==="uncertain"&&bestEligible)reasons.push("Usable and non-input icons looked too similar; review the suggested material before import");else if(relevance==="uncertain")reasons.push("No usable Recipe Book input was identified automatically; search the material catalog");if(relevance==="eligible"){if(!matchedIcon||!options.length)reasons.push("No Recipe Book material matched this icon");else if(borderGradeConflict)reasons.push(borderGradeMatches.length?`The detected ${recipeBookOcrBorderGradeLabel(slot.borderGrade)} border still matches multiple materials`:`The detected ${recipeBookOcrBorderGradeLabel(slot.borderGrade)} border conflicts with this icon's verified quality family`);else if(materialExactResources.length>1)reasons.push("This icon is shared by multiple materials or enhancement levels");else if(matchedIcon.score<RECIPE_BOOK_OCR_EXACT_ICON_MIN_SCORE)reasons.push("The icon match is low confidence");else if(gap<RECIPE_BOOK_OCR_EXACT_ICON_MARGIN)reasons.push("Two icons look nearly identical")};if(slot.quantityApproximate)reasons.push("The displayed K/M quantity is rounded and needs confirmation");else if(slot.quantityAssumedOne)reasons.push("The quantity was hidden and cannot be assumed");else if(readQuantity===null)reasons.push("No valid quantity was detected");else if(!Number.isSafeInteger(slot.quantityValue)||slot.quantityConfidence<.78)reasons.push("The quantity read from the screenshot needs confirmation");
    const row={id:`${analysis.imageFingerprint}:${slot.id}`,fingerprint:analysis.imageFingerprint,slotId:slot.id,row:slot.row,column:slot.column,box:slot.box,options,selectedKey:defaultOption?.key||"",quantity:readQuantity??"",suggestedQuantity:readQuantity,quantityText:slot.quantityText,quantityApproximate:Boolean(slot.quantityApproximate),borderGrade:slot.borderGrade,borderGradeConfidence:slot.borderGradeConfidence,relevance,reviewRequired:!ready,state:ready?"ready":options.length?"review":"unknown",reasons};
    (relevance==="negative"?ignoredRows:rows).push(row);
  }
...
function recipeBookOcrRemoveReviewRow(rows,rowId){const source=Array.isArray(rows)?rows:[],id=String(rowId??""),index=id?source.findIndex(row=>String(row?.id??"")===id):-1;return index<0?[...source]:[...source.slice(0,index),...source.slice(index+1)]}
...
function recipeBookOutputYieldLabel(recipe){
  if(!Number.isSafeInteger(recipe?.outputQuantityMin)||!Number.isSafeInteger(recipe?.outputQuantityMax))return "";
  const amount=recipe.outputQuantityMin===recipe.outputQuantityMax?`×${recipeBookFormatCount(recipe.outputQuantityMin)}`:`×${recipeBookFormatCount(recipe.outputQuantityMin)}–${recipeBookFormatCount(recipe.outputQuantityMax)}`;
  return `Yields ${amount}${recipe.outputQuantityNote?` · ${recipe.outputQuantityNote}`:""}`;
}
...
function recipeBookOcrRefreshSelection(){
  ...
  if(recipeBookEl.screenshotSelectionSummary)recipeBookEl.screenshotSelectionSummary.textContent=plan.valid?`${total} material${total===1?"":"s"} ready to ${mode==="add"?"add":"update"}.`:total?`${complete} of ${total} rows complete · ${plan.errors[0]}`:state.ignoredRows.length?`${state.ignoredRows.length} strong non-material match${state.ignoredRows.length===1?"":"es"} hidden · review if needed.`:state.images.length?"No usable Recipe Book materials remain in this scan.":state.warnings.length?"Review the scanner message above.":"Add a screenshot to begin.";if(recipeBookEl.screenshotApply)recipeBookEl.screenshotApply.disabled=state.busy||!plan.valid||!addConfirmed;
}
async function recipeBookOcrQueueFiles(inputFiles,source="browse"){
  ...
  for(let index=0;index<queue.length;index++){
    ...
    try{
      ...
      const registered=recipeBookOcrRegisterFingerprint(state.fingerprints,analysis.imageFingerprint);if(registered.duplicate){state.warnings.push(`${displayName} was skipped because the same screenshot is already in this session.`);continue}const partition=recipeBookOcrPartitionReviewRows(analysis,recipeBookState.data),reviewRows=partition.rows,ignoredRows=partition.ignoredRows;state.fingerprints=[...registered.fingerprints];state.totalBytes+=normalized.bytes;state.images.push({fingerprint:analysis.imageFingerprint,fileName:normalized.originalName,dataUrl:normalized.dataUrl,width:normalized.width,height:normalized.height,imagePromise:normalized.imagePromise});state.rows.push(...reviewRows);state.ignoredRows.push(...ignoredRows);state.warnings.push(...analysis.warnings.map(warning=>`${displayName}: ${warning}`));if(!analysis.slots.length)state.warnings.push(`${displayName}: no storage material slots were detected. Include one or more complete item slots with their quantity labels, or crop tightly around a single item.`);else if(!reviewRows.length)state.warnings.push(`${displayName}: every detected slot strongly matched a known non-material item. Review the hidden slots if a usable material was missed.`);accepted++;
    }catch(error){if(token!==state.generation)return;state.warnings.push(error?.message||`${displayName}: analysis failed.`)}
  }
  if(token!==state.generation)return;if(state.controller===controller)state.controller=null;recipeBookOcrSetBusy(false);recipeBookOcrSetStatus(state.images.length?`${state.images.length} screenshot${state.images.length===1?"":"s"} · ${state.rows.length} material slot${state.rows.length===1?"":"s"} to review${state.ignoredRows.length?` · ${state.ignoredRows.length} strong non-material match${state.ignoredRows.length===1?"":"es"} hidden`:""}`:"No screenshots were accepted. Review the messages below.",state.images.length?"ready":"error");recipeBookOcrRenderReview();if(accepted||state.warnings.length)recipeBookEl.screenshotReview?.scrollIntoView({block:"nearest",behavior:document.body.dataset.motion==="reduced"?"auto":"smooth"});
}
...
function initializeRecipeBook(){
  ...
  recipeBookEl.screenshotRows?.addEventListener("click",event=>{const remove=event.target.closest("[data-ocr-row-remove]");if(remove){event.preventDefault();const state=recipeBookState.ocr,before=state.rows.length,removedIndex=state.rows.findIndex(row=>row.id===remove.dataset.ocrRowRemove);state.rows=recipeBookOcrRemoveReviewRow(state.rows,remove.dataset.ocrRowRemove);if(state.rows.length<before){state.removedSlots++;recipeBookOcrCloseMaterialPopups();const omitted=state.ignoredRows.length+state.removedSlots;recipeBookOcrSetStatus(`${state.images.length} screenshot${state.images.length===1?"":"s"} · ${state.rows.length} material slot${state.rows.length===1?"":"s"} to review${omitted?` · ${omitted} hidden or removed`:""}`,state.rows.length?"ready":"idle");recipeBookOcrRenderReview();requestAnimationFrame(()=>{const buttons=[...recipeBookEl.screenshotRows?.querySelectorAll("[data-ocr-row-remove]")||[]],target=buttons[Math.min(Math.max(removedIndex,0),buttons.length-1)]||recipeBookEl.screenshotReviewIgnored||recipeBookEl.screenshotClear||recipeBookEl.screenshotCancel;if(target&&!target.hidden)target.focus()})}return}const article=event.target.closest("[data-ocr-row-index]"),row=article&&recipeBookState.ocr.rows[Number(article.dataset.ocrRowIndex)];if(!row)return;const candidate=event.target.closest("[data-ocr-material-option]");if(candidate){event.preventDefault();recipeBookOcrSelectMaterial(article,row,candidate.dataset.ocrMaterialOption,"candidate");return}const searchResult=event.target.closest("[data-ocr-search-option]");if(searchResult){event.preventDefault();recipeBookOcrSelectMaterial(article,row,searchResult.dataset.ocrSearchOption,"search");return}const trigger=event.target.closest("[data-ocr-material-trigger]");if(trigger){event.preventDefault();const popup=document.getElementById(trigger.getAttribute("aria-controls")||"");if(popup&&!popup.hidden)recipeBookOcrCloseMaterialPopups();else recipeBookOcrOpenMaterialMenu(trigger)}});
...
function initializeAppView(viewId){
  ...
  if(viewId === "weekliesView"){initializeWeeklies();tickWeeklies()}
