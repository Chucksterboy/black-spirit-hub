...
async function sendNodeWarAlert(title,message,spokenText,settings){const channel=settings.nodeWarNotificationMode==="alarm"?{name:"Alarm.mp3",promise:bridgeCall("playAlarmSound",{})}:settings.nodeWarNotificationMode==="tts"?{name:"text to speech",promise:bridgeCall("speakText",{text:spokenText})}:null;if(!channel)return false;try{await channel.promise;NotificationService.ShowInfo(message,title);return true}catch(error){console.warn("[NodeWarNotify] Alert channel failure",{channel:channel.name,error});NotificationService.ShowError(`Could not deliver the Node War ${channel.name} alert.`,"Node War alert failed");return false}}
...
function runBossTtsTest(){const text=bossTestTtsText();return runBossAlertTest(homeEl.testTts,"Speaking",()=>bridgeCall("speakText",{text}),result=>`English TTS played successfully${result?.voiceName?` with ${result.voiceName}`:""}: ${text}`)}
function runBossAlarmTest(){return runBossAlertTest(homeEl.testAlarm,"Playing",()=>bridgeCall("playAlarmSound",{}),result=>`Alarm.mp3 is playing${result?.durationMilliseconds?` (${(result.durationMilliseconds/1000).toFixed(1)} seconds)`:``}.`)}
