...
function couponSourceAttribution(c){const sources=String(c?.source||"").split("+").map(value=>value.trim()).filter(Boolean),unique=[...new Set(sources)];if(!unique.length)return"";return`<div class="couponDetailSource"><span>SOURCE${unique.length===1?"":"S"}</span>${unique.map(source=>source.toLowerCase()==="garmoth"?'<button type="button" data-open-url="https://garmoth.com/coupons/">Garmoth &nearr;</button>':`<strong>${couponEscape(source)}</strong>`).join('<i aria-hidden="true">+</i>')}</div>`}
