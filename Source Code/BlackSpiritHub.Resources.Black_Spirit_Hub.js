...
const HOME_ALERT_MILESTONES=Object.freeze([5,10,15,30]);
function alertStage(settings,delta,keyBase){const lead=Number(settings.leadMinutes||15);if(delta<0||delta>lead*60000)return null;const stage=HOME_ALERT_MILESTONES.find(minutes=>minutes<=lead&&delta<=minutes*60000),notified=settings.notified||{};return stage&&!notified[`${keyBase}|${stage}`]?stage:null}
...
function migrateLegacyHomeAlert(keyBase,stage,settings){if(!(settings.notified||{})[keyBase])return false;const selectedLead=Number(settings.leadMinutes),deliveredStage=HOME_ALERT_MILESTONES.includes(selectedLead)?selectedLead:stage,latest=normalizedHomeSettings(),notified={...(latest.notified||{}),...(settings.notified||{}),[`${keyBase}|${deliveredStage}`]:true},merged={...latest,notified};delete notified[keyBase];settings.notified=notified;pruneHomeNotifications(merged);saveHomeSettings(merged);return deliveredStage===stage}
async function checkBossNotifications(settings,now,next){if(!settings.masterNotifications||!next)return false;const delta=next.date-now,keyBase=`boss|${next.date.toISOString()}`,stage=alertStage(settings,delta,keyBase);if(!stage||migrateLegacyHomeAlert(keyBase,stage,settings))return false;const key=`${keyBase}|${stage}`,warningMinutes=Math.max(1,Math.ceil(delta/60000));console.debug("[HomeBossNotify]",{bosses:next.bosses,spawnUtc:next.date.toISOString(),deltaMs:delta,milestoneMinutes:stage,warningMinutes,leadMinutes:settings.leadMinutes,serverTime:fmtSpawnDateTime(next,{...settings,showLocalTime:false}),localTime:fmtSpawnDateTime(next,{...settings,showLocalTime:true})});const title=`${next.bosses.join(" + ")} spawning soon`,message=`${fmtSpawnDateTime(next,settings)} - ${warningMinutes} minute warning - ${HOME_TIMER_CONFIG.region}`,spoken=`${spokenBossList(next.bosses)} spawning in ${alertLeadText(warningMinutes)}.`;return persistDeliveredHomeAlert(key,settings,()=>sendHomeAlert(title,message,spoken,settings))}
async function checkGuildBossNotifications(settings,now){if(!settings.masterNotifications||!settings.guildBossNotifications)return false;const target=guildBossTarget(now);if(!target)return false;const delta=target.date-now,keyBase=`guild|${target.date.toISOString()}`,stage=alertStage(settings,delta,keyBase);if(!stage||migrateLegacyHomeAlert(keyBase,stage,settings))return false;const key=`${keyBase}|${stage}`,warningMinutes=Math.max(1,Math.ceil(delta/60000));const title="Guild bosses spawning soon",message=`${guildBossDayName(target.day)} ${target.time} - ${warningMinutes} minute warning - Weekly guild boss timer`,spoken=`Guild bosses spawning in ${alertLeadText(warningMinutes)}.`;return persistDeliveredHomeAlert(key,settings,()=>sendHomeAlert(title,message,spoken,settings))}
...
Object.assign(GRIND_FIXED_ITEM_PRICES,{"inner-edania-abundance-imbued-branch":155127,"inner-edania-black-crystal-fragment":160539,"inner-edania-elion-follower-helmet":181042,"inner-edania-scorched-belt-ornament":182049,"inner-edania-elion-follower-mark":186458,"inner-edania-broken-void-glove":196501});
...
Object.assign(grindSpotCcOverrides,{"aphrodon temple":["knockdown","bound"],"emesia fortress":["knockback","float"],"magaia temple":["stun","stiffness","freeze"],"aresion temple":["knockdown","bound"],"scales of judgment":["stun","stiffness","freeze"],"event horizon":["stun","stiffness","freeze"]});
Object.assign(grindMaxCapOverrides,{"aphrodon temple":[2090,810],"emesia fortress":[2220,830],"magaia temple":[2340,840],"aresion temple":[2455,850],"scales of judgment":[2455,860],"event horizon":[2570,870]});
...
function grindRenderSpotDetail(){
  ...
  grindEl.spotDetail.innerHTML=`<div class="grindSpotTop"><img src="${escapeHtml(spot.icon||trash?.icon||"")}" alt=""><div><h2>${escapeHtml(spot.name)}</h2><p>${escapeHtml(spot.zone||"Unknown region")} - ${escapeHtml(trash?.name||spot.primaryTrash||"Trash loot")} - ${spot.players||1} player${String(spot.players||"1")==="1"?"":"s"}</p></div><div class="grindSpotStats"><span>${spot.ap||"-"} AP</span><span>${spot.dp||"-"} DP</span></div></div><div class="grindLootGrid">${loot||grindEmpty("No drop table available for this spot.")}</div>${grindRenderResistancePanel(spot)}`;
}
...
function eventsSyncTimelineLabels(scroller=eventsEl.timeline){if(!scroller?.querySelectorAll||!scroller.getBoundingClientRect)return;const viewport=scroller.getBoundingClientRect(),visibleLeft=viewport.left+12;for(const bar of scroller.querySelectorAll(".eventsTimelineBar")){const label=bar.querySelector?.(".eventsTimelineBarText");if(!label)continue;const rect=bar.getBoundingClientRect(),visible=rect.right>viewport.left&&rect.left<viewport.right,shift=visible?Math.max(0,visibleLeft-rect.left):0;label.style.setProperty("--event-label-shift",`${Math.round(shift)}px`)}}
